...
/**
 * H-5 — WHAT A FIELD THAT STATES NO POLARITY POLICY IS, in one sentence with
 * several readers.
 *
 * Absent is the identity: no candidate carries a polarity, both design steps
 * enumerate the combinations on ideal filters and tie-break internally, every
 * label is what it was, and `candidateFieldKey` — and with it every recorded
 * run fingerprint — reproduces byte for byte.
 *
 * THE APP ALWAYS STATES A POLICY SINCE H-5 (its default is `'textbook'`). THE
 * CASUS FIXTURES DELIBERATELY DO NOT, and that is a PIN and not an oversight:
 * every corpus in this casebook was generated by the enumerating design step,
 * and on casus 1b and casus 1h that step chose the MIRRORED arm on every
 * candidate (H-4 measured it: three of three and one of one). A fixture that
 * adopted the H-5 default would design those corpora at a different polarity
 * and their live byte reproductions would stop reproducing — which would not be
 * a regression but the consequence of a rule that arrived after them. The same
 * shape M-3 used for the measurement set: the live chain runs read the stand
 * their corpus was made on, the guard asserts the pin DIFFERS from the default
 * so it cannot go quietly vacuous, and the next regeneration is where they move.
 */
export const POLARITY_ARMS_ABSENT_MEANS =
  'no polarity policy: no candidate carries a stated polarity and the design step enumerates the ' +
  'combinations on ideal filters, exactly as every corpus in this casebook was generated (H-5).';
...
export function polarityLabel(
  ...
  /**
   * H-5 — WHY this is the textbook arm, in the alignment's own name.
   *
   * Sander stated on 24-09-2026 that the rule may be deterministic only if it
   * is said out loud wherever it puts a polarity. A bare `· mid ⌀` is the mark
   * a builder solders by and says nothing about WHO asked for it; `· mid ⌀ ·
   * textbook for LR2` is the same mark with its reason attached, and on an LR4
   * field the reason is simply absent because nothing asked.
   */
  ...
  if (!arm) return ways;
  const why =
    arm === 'textbook' && askedBy !== undefined && askedBy.length > 0
      ? `${arm} for ${askedBy.join(' + ')}`
      : arm;
  return `${ways} · ${why}`;
}

/**
 * The alignments that ASK for a reversal, distinct and in crossing order.
 *
 * Empty on an all-LR4 field, which is why the textbook label there is the bare
 * word: there is no rule to name when no rule fired.
 */
export function alignmentsAskingReversal(crossings: readonly CandidateCrossing[]): string[] {
  const out: string[] = [];
  for (const x of crossings) {
    if (!textbookRelativeInverted(x.alignment)) continue;
    const name = `${x.alignment.kind}${x.alignment.order}`;
...
export interface PolarityArmPolicy {
  /**
   * `'textbook'` mirrors NOTHING: every candidate carries the polarity its
   * alignments ask for and no other arm is built. H-5's default, and the only
   * seed under which the field is DETERMINISTIC — the design step is bound on
   * every candidate and never tie-breaks internally.
   *
   * `'both'` mirrors every handover of every candidate — what the FULL field
   * asks for under the stated `both` choice, because that is what "both" means.
   * `'margin'` mirrors a handover only where `marginFor` says the phase
   * argument does not decide — what an EXPLORATION under that choice asks for,
   * because a mirrored arm is a whole chain run and doubling an exploration by
   * reflex would undo what E-2 bought.
   */
  seed: 'both' | 'margin' | 'textbook';
  /**
...
export function expandPolarityArms(
  ...
  for (const c of field.candidates) {
    const mirror = c.crossings.map((x) => {
      ...
      /* H-5 — the textbook seed mirrors NOTHING, and it says so before every
       * other rule gets a turn: under it the reading is taken (the run notes
       * print what a mirror would have been worth) and it decides nothing. */
      if (policy.seed === 'textbook') return false;
    ...
    /* H-5 — EVERY CANDIDATE OF AN EXPANDED FIELD CARRIES ITS POLARITY, also
     * the one that stands alone. That is the whole of the deterministic rule:
     * a candidate WITHOUT a `polarity` is one whose design step still
     * enumerates and tie-breaks on ideal filters, and H-5 exists to stop that
     * happening on the v2 route. A field with NO policy at all is untouched
     * and still enumerates — absent is the identity, and that is what keeps
     * every corpus recorded before H-5 reproducing (P2). */
    const askedBy = alignmentsAskingReversal(c.crossings);
    for (const p of polarityArmsOf(c.crossings, mirror, guarantee)) {
      ...
      out.push({
         ...
         * H-5 — and the textbook arm names the alignment that asked. */
        label: `${c.label}${polarityLabel(p.invertedWays, p.arm, askedBy)}`,
        polarity: p,
  ...
  const textbookOnly = policy.seed === 'textbook';
  const gated =
    possible > 0 && emitted < possible && !textbookOnly
      ? [
  ...
  const notes =
    textbookOnly
      ? [
          'Polarity: TEXTBOOK, stated on every candidate. Each handover carries the relative ' +
            'polarity its alignment asks for — LR2 one reversal, LR4 none — so the design step is ' +
            'bound and never settles it on an internal tie-break over ideal filters (H-5). ' +
            'The mirrored arm of each handover is NOT built on this run; the pre-design phase ' +
            'reading below says at which crossings it would have been closest, and the stated ' +
            'run choice "polarity arms: both" builds them (H-4/H-4b).',
          ...gated,
        ]
      : added > 0
      ? [
