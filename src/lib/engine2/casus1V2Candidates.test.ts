 ...
 * ONE candidate runs live HERE and the rest are read from disk; the second live
 * run — the candidate a wholesale rule refuses — moved to
 * `casus1V2Refusal.test.ts` on 01-09-2026 so that the two run side by side
 * instead of one after the other. A regression nobody runs because it is slow
 ...
 *
 * Two cases in the whole suite run a real chain — one delivering candidate and
 * one refused one — and at V42 they cost 1427 s and 653 s. That is roughly
 * ninety-nine per cent of the wall clock of `vitest run`, and a suite nobody
 * runs during development protects nothing.
 *
 * SINCE 01-09-2026 THEY ARE TWO FILES, AND THAT IS SCHEDULING TOO.
 * `handleV2Request` is synchronous, so two live chain runs in ONE file run one
 * after the other on a single worker however many cores the machine has; vitest
 * parallelises over FILES. The refused one therefore lives in
 * `casus1V2Refusal.test.ts`, unchanged in content, and the guard at the bottom
 * of this file expects exactly TWO tagged blocks by name.
 *
...
describe('the live-run tag is a schedule, not a hiding place', () => {
  ...
  it('exactly two blocks carry it, and both are live chain runs', () => {
     ...
     * The names are written out so that a third one is a deliberate act. */
