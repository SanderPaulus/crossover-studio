...
export const HELP_SECTIONS_EN: HelpSection[] = [
  {
    ...
    blocks: [
      ...
      },
      {
        t: 'p',
        text:
          'No measurements of your own at hand? **Load KOAN demo data** (Import tab) loads a complete real measurement set incl. angles, plus the priced demo catalog (only if you have not imported a catalog of your own). **Load 3-way demo** loads the same loudspeaker three years on, finished: woofer pair (measured together), mid and tweeter at 0–60°, near-field cones + port, LIMP impedances and the cabinet/rig as entered — the full 3-way flow without a VituixCAD project.',
      },
    ],
  },
  {
    ...
    blocks: [
      ...
      { t: 'h', text: '⚙ Settings (also reachable through the wizard)' },
      {
        ...
        items: [
          ...
          '**Without a pin the 3-way scan searches a physics window per handover** — the INTERSECTION of every limit, and ⚙ Settings names, per edge, the rule that set it: (1) the **data floor** 2/gate from the file header or the gate field (a spliced branch gets the top of its splice blend as its floor — no handover inside the merge), which beats everything including a pin — a candidate below the data floor is designed on noise, so the app clamps and says "measure lower (ground plane/near field)"; (2) **array lobing** for several drivers in one branch (k·c/spacing, k 0.5 = λ/2); (3) **centre-to-centre** of the pair (λ/N, automatic by axis: vertical stack λ/1, side by side λ/2); (4) the lower driver\'s **breakup** / 1.8; (5) **K × in-situ fs** of the upper driver from the ZMA (2 with an LCR trap, 3 without); (6) the excursion floor; plus the measured beaming onset and where the upper driver reaches level. When two limits collide the app says so (no silent hop to another region) and searches just above the floor. Candidates: the corners, the log-midpoint and — if you already have a design — its current crossings as a warm start. The thresholds sit under the readout.',
    ...
    blocks: [
      ...
      { t: 'h', text: 'Safety nets' },
      {
        ...
        items: [
          ...
          '**Amplifier floor (optional)**: specify your amplifier’s minimum load impedance and the optimizer will treat it as a hard constraint — repairing or flagging designs that dip below it. Leave the field empty and no floor is applied: the engine reports the measured minimum but makes no judgement.',
          '**Part audit (gate 4, physical and absolute)**: the first three gates (band audit in the design step, synthesis corrections, staged prune) judge relative to the objective — and the prune only runs when the targets are met, so with an unreachable target a demonstrably dead part simply stayed. This audit asks one question per part (and per series LCR chain as a whole): does it demonstrably do anything? Opened/shorted without retuning: max |ΔSPL| of the sum (200 Hz–15 kHz), the change of the relative phase where the drivers hand over, and the change of the Z minimum and of the source resistance the low driver sees. **Inert** (< 0.15 dB, < 1.5°, Z unchanged) → removed, targets met or not (🔒 is only reported); **earned** (≥ 1 dB, ≥ 3°, or the Z minimum drops under the floor / the source resistance crosses its limit) → stays with the reason; **grey** → the numbers are in the table, you decide. The table under the tune diff also shows per part the ratio |Z of the part| against |Z it sees| in the band where its branch counts (shunt inert when ≫ 1, series when ≪ 1) and the saving in €. Runs on the seed values and on the tuned network — a free tune would rather rebuild a dead part into something unasked-for than leave it dead. Plus one network-level verdict independent of the parts: the **source resistance at the low driver at its box tuning** (Qes increase): a series resistor or heavy DCR in front of the woofer lowers the damping, and no response metric sees that.',
    ...
    blocks: [
      {
        ...
        items: [
          ...
          '**Export .vxp** (Network tab) writes all design tabs as variants; the active tab becomes the opening schematic. The measurement files always travel with it: in Chromium the button writes a complete folder (the .vxp file WITH all response and impedance files beside it), and in Safari/Firefox it downloads that same folder as a single **.zip** — unpack it and open the .vxp. Either way VituixCAD opens without "files not found".',
