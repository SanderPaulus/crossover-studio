...
export const nl: Record<string, string> = {
  ...
  "Remove this branch — its response, angle files and impedance": "Verwijder deze tak — responsie, hoekbestanden en impedantie",
  "Part audit — {n} parts: {i} inert · {e} earned · {g} grey": "Onderdelen-audit — {n} onderdelen: {i} inert · {e} verdiend · {g} grijs",
  "source R at the low driver {r} Ω @ {f} Hz": "bronweerstand aan de lage driver {r} Ω @ {f} Hz",
  "Each part opened/shorted WITHOUT retuning, measured against the full network: max |ΔSPL| of the sum (200 Hz–15 kHz, 1/6-oct smoothed), the P95 change of the relative phase where the drivers hand over, and the change of the system Z minimum. Inert = removable whether or not the targets are met (locked parts are only reported); earned = it demonstrably works; grey = the numbers are yours to judge. The ratio is |Z of the part| against |Z it sees| over the band where its branch is within 12 dB of the sum — a shunt part is inert when ≫ 1, a series part when ≪ 1.": "Elk onderdeel open/kortgesloten ZONDER hertunen, gemeten tegen het volledige netwerk: max |ΔSPL| van de som (200 Hz–15 kHz, 1/6-oct gesmoothed), de P95-verandering van de relatieve fase waar de drivers overnemen, en de verandering van het systeem-Z-minimum. Inert = verwijderbaar of de doelen nu gehaald zijn of niet (vergrendelde onderdelen worden alleen gemeld); verdiend = doet aantoonbaar iets; grijs = de cijfers zijn aan jou. De verhouding is |Z van het onderdeel| tegen |Z die het ziet| over de band waar zijn tak binnen 12 dB van de som zit — een shunt-onderdeel is inert bij ≫ 1, een serie-onderdeel bij ≪ 1.",
  "function": "functie",
  "ratio": "verhouding",
  "verdict": "oordeel",
  "inert — removed": "inert — verwijderd",
  "inert (locked)": "inert (vergrendeld)",
  "inert": "inert",
  "earned": "verdiend",
  "grey": "grijs",
  "taken at the grid edge — the box tuning lies below the view range; widen it for the value at resonance": "genomen op de rand van het rooster — de kastafstemming ligt onder het weergavebereik; verbreed het voor de waarde op de resonantie",
  "Load the 3-way KOAN 2951 session of Aug 2026: woofer pair (measured together), mid and tweeter at 0–60°, near-field cones + port, LIMP impedances, and the cabinet/rig as entered — the full three-branch flow, no VituixCAD project needed": "Laad de 3-weg KOAN 2951-sessie van aug 2026: wooferpaar (samen gemeten), mid en tweeter op 0–60°, nabij-veld conussen + poort, LIMP-impedanties, en de kast/opstelling zoals ingevuld — de volledige drie-takken-flow, geen VituixCAD-project nodig",
  "Loading the 3-way demo…": "3-weg-demo laden…",
  "3-way demo loaded — {label}: woofer pair, mid, tweeter, 0–60°, near fields and the measured cabinet": "3-weg-demo geladen — {label}: wooferpaar, mid, tweeter, 0–60°, nabij-veld en de gemeten kast",
  "3-way, Aug 2026: woofer pair + mid + tweeter, angles, near fields, cabinet": "3-weg, aug 2026: wooferpaar + mid + tweeter, hoeken, nabij-veld, kast",
  "Load the 2-way demo (KOAN prototype 2023)": "Laad de 2-weg-demo (KOAN-prototype 2023)",
  "mid + tweeter, angles, VituixCAD variants": "mid + tweeter, hoeken, VituixCAD-varianten",
  "Load 2-way demo (KOAN prototype 2023)": "Laad 2-weg-demo (KOAN-prototype 2023)",
  "Load the bundled 2023 KOAN prototype measurements (mid + tweeter, all angles + impedances + vxp variants) — instant playground": "Laad de meegeleverde KOAN-prototype-metingen van 2023 (mid + tweeter, alle hoeken + impedanties + vxp-varianten) — meteen een speeltuin",
  "The free scan searches the INTERSECTION of every physical limit: data floor (2/gate, or the near-field splice), array lobing of a multi-driver branch, centre-to-centre spacing (λ/N), the lower driver's breakup / N and its measured beaming, K× the upper driver's in-situ fs, its excursion floor, and where it reaches level. Each edge names the rule that set it. A pin replaces the physics rules but never the data floor.": "De vrije scan doorzoekt de DOORSNEDE van elke fysische grens: datavloer (2/gate, of de nabij-veld-splice), array-lobing van een meervoudige tak, hart-op-hart-afstand (λ/N), de breakup van de onderste driver / N en zijn gemeten bundeling, K× de in-situ fs van de bovenste driver, zijn excursievloer, en waar hij op niveau komt. Elke rand noemt de regel die hem zette. Een pin vervangt de fysica-regels, maar nooit de datavloer.",
  "rail": "rail",
  "floor": "vloer",
  "ceiling": "plafond",
  "also": "verder",
  "overridden by your pin": "overschreven door je pin",
  "thresholds": "drempels",
  "array k": "array-k",
  "breakup": "breakup",
  "Rule 2 — array lobing ceiling = k · c / spacing of a multi-driver branch. 0.5 = the spacing reaches λ/2 (first forward null).": "Regel 2 — array-lobing-plafond = k · c / afstand van een meervoudige tak. 0,5 = de afstand bereikt λ/2 (eerste voorwaartse nul).",
  "Rule 3 — centre-to-centre ceiling: the spacing between the two drivers of a handover may not exceed λ/N (vertical integration).": "Regel 3 — hart-op-hart-plafond: de afstand tussen de twee drivers van een overgang mag λ/N niet overschrijden (verticale integratie).",
  "Rule 4 — breakup margin: the ceiling of a handover and of every low-pass sits at the lower driver's first breakup / N.": "Regel 4 — breakup-marge: het plafond van een overgang en van elke laagdoorlaat ligt op de eerste breakup van de onderste driver / N.",
  "Rule 5 — resonance floor: the upper driver's handover sits at least K × its in-situ fs (from the ZMA). 2 with an Fs LCR trap in the design, 3 without.": "Regel 5 — resonantievloer: de overgang van de bovenste driver ligt minstens op K × zijn in-situ fs (uit de ZMA). 2 mét een Fs-LCR-trap in het ontwerp, 3 zonder.",
  "Target not realisable: the tuned network crosses more than ⅓ octave from the candidate it aimed at — the window or the topology binds. ": "Doel niet realiseerbaar: het getunede netwerk kruist meer dan ⅓ octaaf naast de kandidaat waarop hij mikte — het venster of de topologie knelt. ",
  "Named after the DELIVERED acoustic crossing; aimed at {target}": "Genoemd naar de GEREALISEERDE akoestische kruising; mikte op {target}",
  "(aim": "(doel",
  "DI match": "DI-aansluiting",
  "inside the window, seeded as a candidate": "binnen het venster, als kandidaat gezaaid",
  "outside the window (shown, not seeded)": "buiten het venster (getoond, niet gezaaid)",
  "Rule 3 — centre-to-centre ceiling: the spacing between the two drivers of a handover may not exceed λ/N. Auto = by axis: a vertical stack λ/1 (its first null lands at ±30° vertical — floor and ceiling, not the listening plane), side by side λ/2 (the null would sit in the listening plane), mixed in between.": "Regel 3 — hart-op-hart-plafond: de afstand tussen de twee drivers van een overgang mag λ/N niet overschrijden. Auto = per as: verticaal gestapeld λ/1 (de eerste nul valt op ±30° verticaal — vloer en plafond, niet het luistervlak), zij-aan-zij λ/2 (de nul zou in het luistervlak zitten), gemengd ertussenin.",
  "auto (by axis)": "auto (per as)",
  "How the 3-way scan searches the two handovers. Axis by axis: sweep W-M (M-T held at its anchor), then M-T with the best W-M, then a local 3×3 refinement around the pair (skipped when the sweeps show no coupling) — finer per axis for far fewer chains than a grid of the same resolution. Grid: every corner combination.": "Hoe de 3-weg-scan de twee overgangen doorzoekt. As-voor-as: sweep W-M (M-T vast op zijn anker), dan M-T met de beste W-M, dan een lokale 3×3-verfijning rond het paar (overgeslagen als de sweeps geen koppeling tonen) — fijner per as voor veel minder ketens dan een rooster met dezelfde resolutie. Rooster: elke hoekcombinatie.",
  "Scan strategy": "Scanstrategie",
  "axis by axis (W-M sweep → M-T sweep → refine)": "as-voor-as (W-M-sweep → M-T-sweep → verfijnen)",
  "grid (corners)": "rooster (hoeken)",
  "Points per axis for the sweeps: 3/5/7 — the W-M sweep runs that many chains, the M-T sweep as many again, the refinement up to 9 more.": "Punten per as voor de sweeps: 3/5/7 — de W-M-sweep draait zoveel ketens, de M-T-sweep nog eens zoveel, de verfijning maximaal 9 extra.",
  "Points per axis": "Punten per as",
  "How the energy average is judged. Smooth (default): the trend of the power response (dB/decade) is fitted and left FREE — a rising DI makes it fall, and the slope is your room correction's business — and only the RESIDUAL counts: its std plus a fold term near each crossing (a DI step no room EQ can undo). Legacy: std of the raw power response (flatness), the pre-Aug-2026 behaviour, for A/B on existing projects.": "Hoe de energy average wordt beoordeeld. Glad (default): de trend van de power response (dB/decade) wordt gefit en VRIJ gelaten — een stijgende DI laat hem dalen, en de helling is het terrein van je kamercorrectie — en alleen het RESIDU telt: zijn std plus een plooi-term rond elk kruispunt (een DI-stap die geen room-EQ kan herstellen). Legacy: std van de rauwe power response (vlakheid), het gedrag van vóór aug 2026, voor A/B op bestaande projecten.",
  "Power response": "Power response",
  "smooth (slope free, fold penalised)": "glad (helling vrij, plooi bestraft)",
  "legacy (flat)": "legacy (vlak)",
  "fold ×": "plooi ×",
  "Weight of the DI-fold term (max |residual| within ×/÷1.6 of a crossing) as a share of the in-room weight. Default 0.5.": "Gewicht van de DI-plooi-term (max |residu| binnen ×/÷1,6 van een kruispunt) als aandeel van het in-room-gewicht. Default 0,5.",
  "Fitted 1st-order trend of the energy average over the visible band (dB per decade) — reported, not optimised: the slope is room/taste territory (a rising DI makes it fall). The residual after detrending is what the in-room weight judges: its std, plus the largest fold within ×/÷1.6 of each crossing.": "Gefitte 1e-orde-trend van de energy average over de zichtbare band (dB per decade) — gerapporteerd, niet geoptimaliseerd: de helling is kamer/smaak-terrein (een stijgende DI laat hem dalen). Het residu na detrending is wat het in-room-gewicht beoordeelt: zijn std, plus de grootste plooi binnen ×/÷1,6 van elk kruispunt.",
  "Power response: slope {s} dB/dec · smoothness (residual std) {r} dB · fold at crossings {f} dB": "Power response: helling {s} dB/dec · gladheid (residu-std) {r} dB · plooi bij kruispunten {f} dB",
  "rising power response — almost always a level or measurement error, not a design choice": "stijgende power response — vrijwel altijd een niveau- of meetfout, geen ontwerpkeuze",
  "Directivity in the STRUCTURE search (3-way): each handover pays weight × log2(knee / DI anchor)², the anchor being where the lower driver's DI meets the upper's (from the angle sets). In the literature directivity match is the first crossover criterion, not an afterthought of the tuner. 0 = off.": "Directiviteit in de STRUCTUURzoeker (3-weg): elke overgang betaalt gewicht × log2(knie / DI-anker)², het anker is waar de DI van de onderste driver die van de bovenste inhaalt (uit de hoeksets). In de literatuur is DI-aansluiting het eerste kruispuntcriterium, niet een naweging van de tuner. 0 = uit.",
  "DI anchor weight": "DI-ankergewicht",
  "structure choice on-axis — load angle data for directivity steering": "structuurkeuze on-axis — laad hoekdata voor directiviteitssturing",
  "no DI match found in the measured band — structure choice on-axis": "geen DI-aansluiting gevonden in de gemeten band — structuurkeuze on-axis",
  "Smoothing of the on-axis and energy-average magnitudes the SEARCH judges (Gaussian in log-f, applied before decimation to the inner grid). Diffraction ripple and measurement noise no filter can fix stop steering the search. Off = legacy raw points. Gates, staged targets and the safety gate always judge the raw full grid; the scan table shows the smoothed peak with the raw peak as tooltip.": "Smoothing van de on-axis- en energy-average-magnitudes die de ZOEKTOCHT beoordeelt (gaussisch in log-f, toegepast vóór de decimatie naar het binnengrid). Diffractierimpel en meetruis die geen filter kan fixen sturen de zoektocht dan niet meer. Uit = legacy rauwe punten. Poorten, staged-doelen en de safety-gate oordelen altijd op het rauwe volle grid; de scan-tabel toont de gesmoothde piek met de rauwe piek als tooltip.",
  "Error smoothing": "Fout-smoothing",
  "off (legacy)": "uit (legacy)",
  ...
  "Breakup margin — candidate WINDOW (3-way scan): the ceiling of a handover sits at the lower driver's first breakup / N. This is a second, deliberately milder margin than the harmonic one above: the card/limits margin (f_b/3) says where the distortion penalty of a breakup lands (third harmonic), the window margin (default 1.8) says how close a handover may come to the breakup itself. Two numbers, two questions.": "Breakup-marge — kandidaat-VENSTER (3-weg-scan): het plafond van een overgang ligt op de eerste breakup van de onderste driver / N. Dit is een tweede, bewust mildere marge dan de harmonische hierboven: de kaart/limieten-marge (f_b/3) zegt waar de vervormingsprijs van een breakup landt (derde harmonische), de venstermarge (default 1,8) zegt hoe dicht een overgang bij de breakup zelf mag komen. Twee getallen, twee vragen.",
  "Breakup margin — candidate window": "Breakup-marge — kandidaat-venster",
  "Rule 4 — breakup margin: set under Driver limits (\"Breakup margin — candidate window\").": "Regel 4 — breakup-marge: instellen onder Drivergrenzen (\"Breakup-marge — kandidaat-venster\").",
  "Group delay of the summed response at three fixed points, with the bulk delay (mic flight, {m} ms median) removed — display only, never optimised. Literature reads a passive multi-way at ~0.5–1.5 ms excess GD in the crossover region; large steps between the three points mean phase rotation across a handover.": "Groepsvertraging van de som op drie vaste punten, met de bulk-delay (mic-vlucht, mediaan {m} ms) verwijderd — alleen weergave, nooit geoptimaliseerd. De literatuur leest een passieve meerweg op ~0,5–1,5 ms excess GD rond de kruising; grote sprongen tussen de drie punten betekenen faserotatie over een overgang.",
  "excess GD": "excess GD",
  "Mid band = log2(M-T crossing / W-M crossing). Below ~2.3 octaves the midrange carries too little of the spectrum for its filters to settle: consider a lower W-M or a higher M-T.": "Mid-band = log2(M-T-kruising / W-M-kruising). Onder ~2,3 octaaf draagt de mid te weinig van het spectrum om zijn filters te laten settelen: overweeg een lagere W-M of een hogere M-T.",
  "mid band {o} oct": "mid-band {o} oct",
  "narrow: consider a lower W-M or a higher M-T": "smal: overweeg een lagere W-M of een hogere M-T",
  "Trend of the horizontal energy average over the visible band (dB per decade). Reported only — the slope carries NO weight in the optimizer (a rising DI makes it fall; the slope is your room correction's business). A RISING slope (> +1 dB/dec) almost always means a level or measurement error — a swapped driver file, a tweeter measured too hot — not a design choice.": "Trend van de horizontale energy average over de zichtbare band (dB per decade). Alleen gerapporteerd — de helling weegt NIET mee in de optimizer (een stijgende DI laat hem dalen; de helling is het terrein van je kamercorrectie). Een STIJGENDE helling (> +1 dB/dec) betekent vrijwel altijd een niveau- of meetfout — een verwisseld driverbestand, een te hard gemeten tweeter — geen ontwerpkeuze.",
  "power slope {s} dB/dec": "power-helling {s} dB/dec",
  "rising: check levels/files (no fx influence)": "stijgend: controleer niveaus/bestanden (geen fx-invloed)",
  "Source resistance at the low driver — not measured for this candidate": "Bronweerstand aan de lage driver — niet gemeten voor deze kandidaat",
  "Source resistance the low driver sees at its box tuning (real part, model estimate outside the measured band). Tiers: yellow ≥ {w} Ω, ranking class lost ≥ {l} Ω, disqualified ≥ {d} Ω": "Bronweerstand die de lage driver ziet op zijn kastafstemming (reëel deel, modelschatting buiten de meetband). Trappen: geel ≥ {w} Ω, ranking-klasse kwijt ≥ {l} Ω, gediskwalificeerd ≥ {d} Ω",
  "DISQUALIFIED": "GEDISKWALIFICEERD",
  "Delivered within 5 % under a physics floor (fs·K / excursion / reach). ": "Geleverd binnen 5 % onder een fysische vloer (fs·K / excursie / reach). ",
  "disqualify ≥": "diskwalificeer ≥",
  "Hard tier: a candidate with at least this much source resistance in front of the low driver is disqualified from the ranking — it stays in the table, struck through, with the reason. 0 = off.": "Harde trap: een kandidaat met minstens zoveel bronweerstand vóór de lage driver wordt uit de ranking gediskwalificeerd — hij blijft in de tabel, doorgestreept, met de reden. 0 = uit.",
  "Dissipation term: a soft objective penalty on series resistance in front of the LOWEST branch — weight × (Rs/Re)² at the level reference (Fb or the Z peak). Steers the tuner away from matching levels by burning power in the woofer branch (efficiency and damping), before the hard tiers have to act. 0 = off (legacy).": "Dissipatieterm: een zachte objective-straf op serieweerstand vóór de LAAGSTE tak — gewicht × (Rs/Re)² op het niveau-referentiepunt (Fb of de Z-piek). Stuurt de tuner weg van niveau-matchen door vermogen in de woofertak te verstoken (rendement en demping), vóór de harde trappen moeten ingrijpen. 0 = uit (legacy).",
  "Dissipation weight": "Dissipatiegewicht",
  "This system plays behind room correction (Dirac Live etc.). The RIPPLE target may then be wider — on-axis residual ripple is corrected by the room system — while the PHASE targets stay exactly as they are: phase tracking across a handover is set passively and no room correction repairs it. Amplitude corrects the room; driver integration does not.": "Dit systeem speelt achter kamercorrectie (Dirac Live e.d.). Het RIMPEL-doel mag dan ruimer — restrimpel op de as corrigeert de kamer — terwijl de FASE-doelen exact blijven: fase-tracking over een overname is passief bepaald en wordt door geen kamercorrectie gerepareerd. Amplitude corrigeert de kamer; driver-integratie niet.",
  "Room correction present": "Kamercorrectie aanwezig",
  "ripple target": "rimpeldoel",
  "amplitude corrects the room, driver integration does not — ripple target now {r} dB, phase target unchanged": "amplitude corrigeert de kamer, driver-integratie niet — rimpeldoel nu {r} dB, fasedoel ongewijzigd",
  "B1 — BOM cap per channel. Above it a scan candidate loses a ranking class (same mechanism as the Z floor and the source-R limit) — a decision, not a weight. 0 = off. Unpriced candidates are never punished; missing prices show as [NO PRICE].": "B1 — BOM-plafond per kanaal. Erboven verliest een scan-kandidaat een ranking-klasse (zelfde mechanisme als de Z-vloer en de bron-R-grens) — een beslissing, geen gewicht. 0 = uit. Ongeprijsde kandidaten worden nooit gestraft; ontbrekende prijzen tonen als [GEEN PRIJS].",
  "BOM cap per channel": "BOM-plafond per kanaal",
  "BOM cap per channel (B1): above it a candidate loses a ranking class. Priced rows only; unpriced parts are listed as [NO PRICE] in the BOM.": "BOM-plafond per kanaal (B1): erboven verliest een kandidaat een ranking-klasse. Alleen geprijsde regels; ongeprijsde onderdelen staan als [GEEN PRIJS] in de BOM.",
  "cap": "plafond",
  "peak ±dB": "piek ±dB",
  "avg dev dB": "gem. afw. dB",
  "phase °": "fase °",
  "Cost vs quality — the knee is yours to pick": "Kosten vs kwaliteit — het knikpunt wijs jij aan",
  "Quality measure on the vertical axis": "Kwaliteitsmaat op de verticale as",
  "{n} non-dominated of {m} priced — filled = Pareto front, ◂ = loaded, ✗ = disqualified; click a point to load it": "{n} niet-gedomineerd van {m} geprijsd — gevuld = Pareto-front, ◂ = geladen, ✗ = gediskwalificeerd; klik een punt om het te laden",
  "Pareto scatter of BOM versus quality": "Pareto-spreiding van BOM tegen kwaliteit",
  "Minimal network: remove the most expensive part poort 4 did not mark EARNED, retune, keep while the staged targets, the fundamentals (crossing, valley, tweeter protection, leak, Z floor) and the source-R limit hold; then try cheaper catalog parts within 25 % of each value. Reports BOM before/after and the quality deltas — nothing is applied until you say so.": "Minimaal netwerk: verwijder het duurste onderdeel dat poort 4 niet VERDIEND noemde, retune, houden zolang de staged-doelen, de fundamentals (kruising, vallei, tweeterbescherming, lek, Z-vloer) en de bron-R-grens overeind blijven; probeer daarna goedkopere catalogusonderdelen binnen 25 % van elke waarde. Rapporteert BOM voor/na en de kwaliteitsverschillen — niets wordt toegepast tot jij het zegt.",
  "Minimizing…": "Afslanken…",
  "Minimize network": "Minimaal netwerk",
  "Minimal network for \"{name}\": BOM {b} → {a} ({s}) · peak {p0} → {p1} dB · phase {q0} → {q1}° · R src {r0} → {r1} Ω": "Minimaal netwerk voor \"{name}\": BOM {b} → {a} ({s}) · piek {p0} → {p1} dB · fase {q0} → {q1}° · R bron {r0} → {r1} Ω",
  "Stopped: {why}. Targets {r} dB / {p}° kept at every step; nothing has been applied to the design.": "Gestopt: {why}. Doelen {r} dB / {p}° bij elke stap gehaald; er is niets op het ontwerp toegepast.",
  "step": "stap",
  "why": "waarom",
  "removed": "verwijderd",
  "substituted": "vervangen",
  "[NO PRICE]": "[GEEN PRIJS]",
  "No part could be removed or swapped without breaking a target or a fundamental — this network is already minimal for these goals.": "Geen onderdeel kon weg of vervangen worden zonder een doel of fundamental te breken — dit netwerk is al minimaal voor deze doelen.",
  "Two-for-one suggestions (not applied):": "Twee-voor-een-suggesties (niet toegepast):",
