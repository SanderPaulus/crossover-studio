...
### V51b — serieweerstand op de laagste weg tot een gesteld maximum, geen pad (03-09-2026, **BREAKING, alleen v2-runs**)

**AANLEIDING.** V51 mat dat X = 1,33 dB en dat de baffle step het niveauwerk doet — de topologie-weigering
vuurde geen enkele keer. Maar dertien van vijftien kandidaten strandden op de vloer (2,05–2,49 Ω tegen 2,60)
en één op de mid-excursie. De oorzaak, gemeten: in het overnamegebied geleiden woofer- en midtak tegelijk,
en een 2,9 Ω-paar naast een 3,4 Ω-mid geeft een systeemminimum onder de vloer; HUIDIG's R8 (3,3 Ω) hield dat
op 3,46 Ω. De vuistregel "nooit een pad op de woofer" gaat over VERZWAKKING met weerstandsnetwerken
(3–8 dB), niet over serieweerstand op DCR-schaal — een luchtspoel met 1 Ω DCR ís fysisch een serieweerstand
van 1 Ω, en die kiest men vaak juist om deze reden. Sander stelt: **max 1,0 Ω serieweerstand op de laagste
weg** (`gestelde_eisen.max_serie_R_laagste_weg_ohm`, naast `geen_niveauwerk_op_laagste_weg: true`, dat
blijft staan omdat de pad-verboden gelden). Niveau-effect −2,6 dB op 2,9 Ω, en dat valt binnen de
onzekerheid van X (woofer_down 4–5 dB te zacht op deze mic-positie; op luisterafstand is X eerder 2–4 dB).

**DE EIS ALS TWEEDE TOESTAND VAN DEZELFDE KETENSLEUTEL (A3j, F4c).** `lowestWayLevelWork` kent naast
`'none'` de waarde `{ kind: 'series-r-max', maxOhm }`: op de bus van de laagste weg mag serieweerstand
staan — als component óf als spoel-DCR; de SOM telt — tot het gestelde maximum; geen L-pad, geen shunt-pad,
geen gebypasste pad. Het getal reist IN de waarde en niet als vierde sleutel: een modus zonder maximum en
een maximum zonder modus betekenen elk niets, en twee sleutels die het eens moeten zijn zijn een paar dat
het oneens kan zijn (`chainChoices`/`choiceKeyGuard` pinnen de lijst op drie, ongewijzigd). De afleiding
(`declareCandidateChainChoices({ lowestWaySeriesRMaxOhm })`) laat het gestelde maximum WINNEN van het
verbod — de smallere uitspraak — en leidt uit niets niets af (P4); het getal beweegt de vingerafdruk.
Absent = huidig gedrag (`'none'` als de eis gesteld is; niets als niet). **Één huis, drie lezers**
(`levelWork.ts`, `level-work/1.1`): de inventaris telt sinds V51b de DCR van de seriespoelen APART naast de
discrete weerstanden (`seriesCoils`, `dcrOhm`, `totalSeriesOhm`), en `levelWorkVerdict` is de ene
vergelijking — onder `'none'` blijft DCR wat overblijft en is geen niveauwerk; onder `series-r-max`
oordeelt het TOTAAL, want een regel die alleen het discrete deel telde zou gehaald worden door de ohm in
het koper te leggen. Welke van de twee de ohm draagt is een BOUWKEUZE van de gebruiker; het rapport zegt
bij elke netlist dat een luchtspoel met deze DCR hetzelfde doet als de component, en de engine besluit er
niets over. **Wat de toestand doet, per stap:** de ontwerpstap trimt zoals altijd (de surplus is er nog)
maar stelt geen shelf-pad voor (`forbidsPads`); de synthese stelt ÉÉN kale serie-R voor, gemaatvoerd op de
trim (`R_s = Z·(1/a − 1)`, de serie-alleen-verzwakking `a = Z/(Z+R_s)`), afgekapt op het maximum als harde
clamp op het zaad, en — sinds de tweede regeneratie — ALTIJD zodra de regel hem toestaat (zie de
regeneratie); de tuner houdt de som onder het maximum als somplafond in de `qes-series-r`-vorm van de box
(`bounds.ts`, regel `stated-series-r`: geen inversie maar het gestelde getal, DCR eerst van het budget af,
de vrije weerstanden delen de rest); en de worker weigert wat eroverheen gaat.

**DE WEIGERINGEN (V31-vorm), drie stuks onder de gecapte regel.** (a) Het geleverde netwerk OVERSCHRIJDT
de regel — een pad, of een totaal boven het maximum (wat alleen kan door een DCR die de box niet zag, een
catalogus-snap ná de box, of een pad dat de synthese niet mocht leggen): `by: 'stated-topology'`,
`kinds: ['topology']`, een bevinding over de reparatie. (b) De rimpeldoel-weigering van V51 vuurt onder de
cap alleen als het geleverde netwerk AAN de cap staat: een gemiste rimpel met ruimte onder de cap is van de
tuner, niet van de regel. (c) Een vloerweigering van de poort krijgt **Y** erbij — *"vraagt Y Ω
serieweerstand op de laagste weg tegen gesteld maximum 1,0"* — en `topology` als tweede kind zodra Y boven
het maximum ligt: twee regels waren betrokken en `kinds` zegt dat. Y wordt gemeten op het GEWEIGERDE
netwerk (`rejectedParts`, V31): een weerstand aan de KOP van de weg (`withSeriesResistanceInFront`: één knoop
erbij tussen de hete generatorknoop en het eerste serie-element dat alleen deze weg voedt — daar zitten een
discrete serieweerstand en de DCR van de eerste spoel fysiek), gebisecteerd tot dezelfde M-B/|Z|-poort slaagt
(`evaluateGates`, tolerantie erin — nooit een tweede afleiding, A3g), bereik 20 Ω. **De eerste versie zette
die weerstand direct VÓÓR DE DRIVER, achter de shunt-C van de laagdoorlaat, en dat is gemeten vóór het
opgeschreven is: op élke casus-1-netlist onder de vloer ZAKTE het systeemminimum ermee (V30_KAND_1: 2,447 →
1,102 Ω met 20 Ω "ervoor")** — een ladder waarvan de afsluiting resistief wordt gemaakt resoneert in zijn
eigen shunt-C. Aan de kop tilt hij de hele weg op. En de probe zegt bij `null` altijd WAAROM (onoplosbaar,
geen bus, of "zelfs 20 Ω laat het systeem op X Ω — het minimum zit in een andere weg"), want een kale `null`
kostte het eerste V51b-veld zijn diagnose. Y staat ook op elke GELEVERDE kandidaat (dan het geleverde
totaal: de vloer vraagt niet meer) en in de kolom `levelWork` (`maxSeriesOhm`, `verdict`,
`floorNeedsSeriesOhm`, `floorOhm`). **Sanity op HUIDIG (item 5):** HUIDIG draagt R8 3,30 Ω plus 0,46 Ω
spoelkoper (V45's 3,756 Ω padweerstand) en valt dus BUITEN de eis — bedoeld, HUIDIG is de parallelle
referentie mét pad; het rapport VLAGT (`FLAG (no gate)`), geen poortoordeel beweegt, en de recorder noemt
alle drie de referentiefilters onder `referentiefilters_buiten_eis`. Geen versoepeling (V42-les).

**DE REGENERATIE (2487 s = 41 min, `V2_JOBS=8`; het V51-corpus vooraf bevroren als `V51-KAND-1`,
`v51_corpus`), en zij is TWEE KEER gedraaid.** De eerste run is na acht kandidaten (~25 min) afgebroken
omdat de shards twee gebreken lieten zien: de vijf 396,7-kandidaten kregen géén serie-R (de synthese stelde
hem, in de L-pad-vorm, alleen voor bij een trim onder −0,5 dB — en juist die kandidaten strandden op de
vloer met niets om haar mee op te tillen), en Y was op elke vloerweigering `null` zonder reden. Beide
gerepareerd (de kale serie-R wordt voorgesteld zodra de regel hem toestaat — een weerstand die de tune niet
nodig heeft snoeit de audit zelf weg; de probe geeft zijn reden terug) en de run opnieuw gestart. Dezelfde
vijftien kandidaten, dezelfde gates en budgets, plus de gecapte regel. **Uitkomst: ZES van vijftien
geleverd (V51: één); negen leverden niets.**

  | kandidaat (W-M · M-T) | V51: uitkomst | V51b: uitkomst | serie-R woofer (Ω) | Y — wat de vloer vraagt (Ω) | geweigerde/geleverde tune: min \|Z\| / RMS / M-C slechtste |
  | --- | --- | --- | --- | --- | --- |
  | 396,7 · 1294 | verworpen: \|Z\| 2,10 + tweeter −14,6 | verworpen: **alleen tweeter −13,2** (vloer gehaald) | 1,00 (cap) | 1,00 | 2,58 / 1,27 / −13,2 |
  | 396,7 · 1491,4 | verworpen: \|Z\| 2,16 + tweeter −15,8 | verworpen: **alleen tweeter −13,1** (vloer gehaald) | 0,98 | 0,98 | 2,57 / 0,99 / −13,1 |
  | 396,7 · 1719 | verworpen: \|Z\| 2,21 | verworpen: \|Z\| 2,45 | 0,63 | — (20 Ω laat 2,30 Ω: minimum in mid/tweeter) | 2,51 / 1,95 / −23,9 |
  | 396,7 · 1981,2 | verworpen: \|Z\| 2,29 | **GELEVERD** (`KAND_V2_5`) | 0,87 | 0,87 | 2,59 / 1,04 / −33,7 |
  | 396,7 · 2283,5 | verworpen: \|Z\| 2,13 | **GELEVERD** (`KAND_V2_4`) | 1,00 (cap) | 1,00 | 2,59 / 0,80 / −28,3 |
  | 466,5 · 1294 | verworpen: \|Z\| 2,41 + mid −9,0 + tweeter −14,1 | verworpen: \|Z\| 1,94 + mid −10,7 + tweeter −8,7 | 1,00 (cap) | — (20 Ω laat 1,80 Ω) | 2,16 / 2,50 / −8,7 |
  | 466,5 · 1491,4 | verworpen: mid −11,4 tegen −11,8 | verworpen: \|Z\| 2,49 + tweeter −19,5 | 1,00 (cap) | — (20 Ω laat 2,26 Ω) | 2,50 / 2,50 / −19,5 |
  | 466,5 · 1719 | **GELEVERD** (`V51_KAND_1`): 2,57 / 0,88 / −25,7 | verworpen: **mid −3,7 tegen −12,1** (vloer gehaald) | 0,86 | 0,86 | 2,59 / 1,87 / −3,7 |
  | 466,5 · 1981,2 | verworpen: \|Z\| 2,41 | **GELEVERD** (`KAND_V2_6`) | 0,87 | 0,87 | 2,59 / 1,93 / −23,2 |
  | 466,5 · 2283,5 | verworpen: \|Z\| 2,37 | **GELEVERD** (`KAND_V2_1`) | 1,00 (cap) | 1,00 | 2,73 / 0,61 / −30,9 |
  | 548,5 · 1294 | verworpen: \|Z\| 2,05 + mid +2,6 + tweeter −17,7 | verworpen: \|Z\| 2,53 + mid +0,5 + tweeter −10,6, **`gate, topology`** | 1,00 (cap) | **1,03 — boven het maximum** | 2,59 / 1,91 / +0,5 |
  | 548,5 · 1491,4 | verworpen: \|Z\| 2,36 + tweeter −17,9 | verworpen: \|Z\| 2,06 + mid −1,9 + tweeter −15,1 | 1,00 (cap) | — (20 Ω laat 2,05 Ω) | 2,37 / 2,54 / −1,9 |
  | 548,5 · 1719 | verworpen: \|Z\| 2,42 | **GELEVERD** (`KAND_V2_2`) | 0,97 | 0,97 | 2,87 / 0,61 / −24,0 |
  | 548,5 · 1981,2 | verworpen: \|Z\| 2,41 | verworpen: **alleen mid +1,5 tegen −12,6** (vloer gehaald) | 0,98 | 0,98 | 2,59 / 1,79 / +1,5 |
  | 548,5 · 2283,5 | verworpen: \|Z\| 2,49 | **GELEVERD** (`KAND_V2_3`) | 1,00 (cap) | 1,00 | 2,89 / 0,65 / −31,7 |

**HET ANTWOORD OP DE HOOFDVRAAG.** Van de dertien V51-vloerweigeringen komen er **NEGEN boven 2,6 Ω met
≤ 1,0 Ω**: zes worden geleverd en drie (396,7 · 1294, 396,7 · 1491,4, 548,5 · 1981,2) halen de vloer maar
strandden daarna op M-C — twee op de tweeter (−13,2 / −13,1 tegen −20) en één op de mid (+1,5 tegen de
afgeleide −12,6). **VIER komen er niet**, en Y zegt per kandidaat waarom: op 396,7 · 1719, 466,5 · 1294 en
548,5 · 1491,4 laat zelfs 20 Ω aan de kop van de woofer het systeem op 2,30 / 1,80 / 2,05 Ω staan — **het
minimum zit daar in de mid- en tweetertak**, en geen serieweerstand op de woofer (en dus ook geen actieve
LF-tak) haalt het naar 2,6; op 548,5 · 1294 vraagt de vloer 1,028 Ω, 0,03 boven het maximum, en dát is de
ene kandidaat waar de regel zelf tussen hem en de vloer staat (`kinds: ['gate', 'topology']`). De cap
BINDT: vier van de zes geleverde netlists staan op 1,00 Ω, de andere twee op 0,87 en 0,97. En de ene
V51-overlevende (466,5 · 1719) is VERLOREN: met de weerstand in het wooferpad legde de synthese de mid
anders en die leest nu −3,7 dB op M-C tegen −61,8 bij V51 — het veld loopt onder elke regel een ander pad
(V47b-les), en het levende corpus deelt met het V51-corpus GEEN ENKEL PAAR.

**WAT HET KOST — de gepaarde lezing tegen V50 (vijf paren, `compare-corpora.ts v50 live`), want V51 → levend
heeft geen paar en elk gemiddelde daarover is compositie.** V50's netlists droegen 3,45–4,27 Ω in het
wooferpad; dezelfde vijf kandidaten dragen nu 0,87–1,00 Ω. Gepaard: **RMS 0,84 → 1,01 dB** (per paar 0,67 →
1,04, 0,72 → 0,80, 1,73 → 1,93, 0,55 → 0,61, 0,51 → 0,65 — overal slechter, +0,17 dB gemiddeld), **M-K
9,50 → 12,58° (W-M) en 5,11 → 7,04° (M-T)**, SPL-venster ±1,1–3,0 → ±1,4–3,8 dB. **Opslingering −0,68 →
−0,68 dB, onveranderd** (het budget van 1,4 bijt nergens, 0 van 6), en **lift 4,40 → 1,71 dB**: de
resistieve component, precies zoals V43 het voorspelde — minder serieweerstand, minder lift; de resonante
helft beweegt niet. Totale seriespoel woofer 3,78 → 2,51 mH (minder padweerstand, dezelfde hoek vraagt
minder inductie, V51). **Dissipatie 55,3 → 37,8 %, heetste weerstand 29,0 → 19,4 W bij 100 W en 2,90 →
1,94 W bij 10 W thermisch** (M-A/part 0 van 6 eroverheen), **Q_es× 1,84–2,40 → 1,29–1,33**, min |Z| 2,66–4,17
→ 2,59–2,89 Ω (de vloer gehaald, drie van zes binnen de meettolerantie op 2,59), M-C op de slechtste weg
−27,8 → −29,6 dB gepaard (0 van 6 falend), lobing −3,0 → −3,1 dB. Tegen het V51-corpus (compositie, één
netlist): RMS 0,88 → 0,61–1,93, lift 0,00 → 1,60–1,79 dB, Q_es× 1,01 → 1,29–1,33, dissipatie 35 → 25–48 %,
heetste weerstand 1,98 → 1,20–2,35 W bij 10 W. Correctiegroepen: series-pad 2 → 15 (de zes woofer-R's plus de
pads op mid en tweeter), shunt-pad 0 → 4 (mid/tweeter), damped-trap 1 → 9.

**DE BALANS VOOR PASSIEF-OF-HYBRIDE, met de drie routes ernaast — geen besluit, Sander beslist.**
*DCR-schaal (dit, gebouwd en gemeten):* opent zes van vijftien tegen één bij V51, voor +0,17 dB RMS en +2–3°
fase gepaard tegen de gepadde V50-ontwerpen, met 1,6–1,8 dB resistieve lift (V50: 4,4; V51: 0), Q_es× 1,3
(V50: 2,4; V51: 1,01) en 1,2–2,4 W in de heetste weerstand bij 10 W; de cap bindt op vier van zes, en −2,6
dB niveau op de woofer is ongeveer wat X op luisterafstand vraagt. *Serie (niet gebouwd — de woofers
blijven parallel):* +6,02 dB niveau en impedantie ×4 op het paar — dan is de woofer weer het anker en is de
vloer geen wooferkwestie meer; wat het met de vier mid/tweeter-minima doet is dezelfde vraag als bij
hybride. *Hybride (actieve LF-tak):* haalt de wooferweg uit het passieve netwerk en daarmee de dertien
vloerweigeringen van V51 weg — behalve de vier waarvan het minimum in de mid- en tweetertak zit (1,80–2,30 Ω
zonder woofer), want die blijven onder 2,6 óók zonder woofer, en dat is een bevinding die V51b als
bijvangst van Y oplevert. Wat in alle drie de routes staat: de M-C-weigeringen op de mid (466,5 · 1719,
548,5 · 1981,2 — de mid wordt zachter gepad en harder aangedreven zodra de woofer omlaag mag) en op de
tweeter bij de lage M-T-kruispunten, en die zijn geen vloerkwestie.

**WAT ER NIET VERANDERD IS.** Vloer 2,6, opslingeringsbudget, Q_es-budget, plateau, M-C en weerstandspoort
staan zoals V51 ze liet; geen L-pad of shunt-pad op de laagste weg; geen serie-afleiding (woofers parallel);
geen v2-default die een casus-1-getal is (het maximum staat in het manifest en in het formulierveld, nergens
in `src/lib/engine2/`). In de draaiende app gecontroleerd: de selectie "series R up to a maximum, no pad"
toont het veld "Max series R on lowest way (Ω)", zonder getal waarschuwt het formulier ("reads as not
stated") en met 1,0 leest de sectie "The project allows series resistance on the lowest way up to 1.00 Ω in
total … a build choice"; console schoon.

**TESTS.** `levelWork.test.ts` (+1: 0,5 Ω discreet achter 0,6 Ω DCR is 1,1 Ω en overschrijdt 1,0; een
shunt-pad faalt ongeacht de som; de spoel alleen is onder `'none'` `none`), `lowestWayLevelWork.test.ts`
(+5, waarvan Y MET DE HAND op een papieren netwerk met vlakke resistieve drivers — 0,98 · 6 − 4 = 1,88 Ω
extra, 0 bij een gehaalde vloer, de REDEN bij een vloer boven de tweeter — en Y op de route: op de kleine
fixture zit het minimum niet in de woofer, de probe zegt dat, en `topology` komt er dan niet bij),
`frozenNetlistGates` (V51-blokken op de REGEL; +1 sanity: HUIDIG buiten de eis, gevlagd en niet gepoort),
`casus1V2Candidates` (de meetopstelling leest de regel), `corpusPairing` (+1, herankerd op het gedateerde
V50 → V51: nul paren, de gepaarde lezing zegt n = 0 en drukt geen delta af), `chainChoices`/`choiceKeyGuard`
(de lijst op drie), `toggleRegression`, `f4cRegression`/`workerRouteRegression` (byte-identiek: P2),
`p6Lint` (de probegrenzen als `P6-OK`), `noWeights`, `goldenClassification` (V51-corpus), `ciLayer`.
`tsc -b` groen (ook `scripts/`), productiebuild groen. **Snelle laag (03-09-2026, alleen gedraaid, direct
ná de regeneratie): 398 s, 147 geslaagd + 1 overgeslagen bestand, 1654 geslaagd + 2 overgeslagen tests, in
één keer groen** — geen nieuw bestand, +13 tests (1 + 5 + 1 + 1 nieuwe claims en +5 uit de `it.each` van
`casus1V2Candidates`, die met het levende corpus van één naar zes ging); GEEN nieuwe referentie (V43: 289 s).
**Volle run 03-09-2026: 148 bestanden, 1656 tests, 1182 s (19 min 42), niets overgeslagen, in één keer
groen** — de byte-reproductie van `KAND_V2_1` (466,5 · 2283,5) kostte 1177 s (in de generator 1261 s), de
verwerpingsrun ernaast 1067 s; korter dan V51 (1530 s) hoewel het corpus zesmaal zo groot is, want de live
reproductie treft een andere kandidaat.

**OPENSTAAND.** (1) De vier kandidaten waarvan het minimum in de mid- en tweetertak zit: dat is een vraag
over de HP-ladder van de mid (V32 mat de V30-minima op 82 Hz, in de shunt-L van de mid), en geen enkele van
de drie routes raakt haar. (2) Een cap die op vier van zes bindt is een cap die de vraag stelt of 1,0 het
getal is — Sander stelt; de tabel geeft per kandidaat Y. (3) Y als zoekgrens in plaats van als rapportage:
...
### A5e.3c — het veld op de nieuwe grenzen: 24 kandidaten onder de A5e.3b-grenzen, de weigering op de eigen kruispunten, en de shortlist-grootte als derde toestand (05-09-2026, **BREAKING, alleen v2-runs**)

**AANLEIDING EN OMVANG.** A5e.3b sloot vier grenzen zonder te regenereren en parkeerde de twee live ketenruns
met een pin; deze sessie wekt het veld erop opnieuw op — Sanders keuze uit de ablatietabel is de BOUWBARE val
(L op de spanwijdte van de gestelde 1,4 mm-familie geklemd, 22,0 mH; stapel niet gesteld), en dat is precies wat
de engine sinds A5e.3b (b)1 al doet, dus er is geen instelling bewogen. Vooraf is het A5e.3-veld-corpus bevroren
als `A5E3VELD_KAND_1..7` (`a5e3veld_corpus`, klasse-B-blokken mee, reden in `DATED_REASON`) en is `corpusPairing`
erop herankerd; `measure-a5e3-field.ts` leest zijn A5e.3-veld-rijen sindsdien uit dat gedateerde corpus (de
`compare-corpora`-les van V33: een script met zijn ná-helft hard op het levende corpus maakt na de eerste
regeneratie stilletjes een andere tabel). Regeneratie op het veiligheidsraster-verlengd (`safety-extended`,
394 punten, 10,1–20 317 Hz), `V2_JOBS=8`, budget 24: W-M 148–550 Hz in ACHT posities (147,9 / 178,4 / 215,2 /
259,6 / 313,2 / 377,8 / 455,7 / 549,7), M-T 1647 / 1948 / 2304 (de vloer `drive-stated`: het gestelde −20 dB bij
orde 4), LR4 alleen; plateau 0, `'none'`, gemergede set, families gesteld, vloer 2,6, opslingering 1,4, Q_es 2,4,
M-C tweeter −20 / mid afgeleid, weerstandspoort 10 W — ongewijzigd. De generator schrijft sinds deze sessie het
TWAALFDE besluit in de meetopstelling (`spoel_spanwijdte_plafond_mH`, uit de ketenverklaring: 22,0).

**STAP 1 — DE REGENERATIE: 24 kandidaten, `V2_JOBS=8`, 17 884 s (4 u 58), VEERTIEN geleverd, TIEN geweigerd.**
Per kandidaat 1410–14 690 s (mediaan ~2900 s; de drie duurste 10 961, 11 634 en 14 690 s, alle drie geleverde
kandidaten met een lage W-M-kruising en 700 000–900 000 evaluaties). De eerste golf was na 29 min leesbaar (de
V51b-les): de barrière las het verlengde raster, élke spoel op de laagste weg lag binnen de spanwijdte — op acht
van de veertien geleverde netwerken staat de valspoel EXACT op 22,00 mH, dus de cap bindt — en de vloer werd op
koper alleen gehaald. De uitkomst per positie (`kandidaat_uitkomst`, ná de herhaling van stap 2 — de drie cellen "eigen kruispunten" waren in de eerste run GELEVERD op 2,60 / 2,63 / 2,61 Ω):

| W-M \ M-T | 1647 | 1948 | 2304 |
| --- | --- | --- | --- |
| 147,9 | vloer 2,39 (Y —) | vloer 2,23 (Y —) | vloer 2,52 (Y —) |
| 178,4 | **GELEVERD** 2,58 Ω | topologie (rimpel 2,58 tegen 2,5; X = 0,78) | vloer 2,51 (Y —) |
| 215,2 | topologie (rimpel 3,75; X = 0,78) | eigen kruispunten: tweeter −19,9 (zaad −20,10) — stap 2 | **GELEVERD** 2,59 Ω |
| 259,6 | **GELEVERD** 2,59 Ω | **GELEVERD** 2,58 Ω | **GELEVERD** 2,58 Ω |
| 313,2 | **GELEVERD** 2,57 Ω | eigen kruispunten: tweeter −19,8 (zaad −20,14) — stap 2 | topologie (rimpel 4,60; X = 0,78) |
| 377,8 | **GELEVERD** 2,59 Ω | **GELEVERD** 2,55 Ω | **GELEVERD** 2,61 Ω |
| 455,7 | topologie (L+R-wees L5 16,50 mH + R7 24,33 Ω, level-work/1.2) | eigen kruispunten: tweeter −19,6 (zaad −20,19) — stap 2 | topologie (rimpel 4,15; X = 0,78) |
| 549,7 | **GELEVERD** 2,59 Ω | topologie (L+R-wees L8 11,41 mH + R10 1,29 Ω) | **GELEVERD** 2,57 Ω, niet bevroren (stap 3) |

Drie dingen vallen op vóór er een tabel bij komt. (1) **De laagste positie (147,9) is compleet weg, en niet op de
spoelen**: alle drie op de vloer (2,23–2,52 Ω) met Y ONOPLOSBAAR — het minimum zit in de mid- of tweetertak — en
op 147,9 · 1647 staat de valspoel op 22,00 mH, de cap. (2) **De rimpeldoel-weigering (V51, X = 0,78 dB) vuurt vijf
keer**, op 178,4 · 1948 met 0,08 dB (2,58 tegen 2,5), en de L+R-wees van A5e.3b (c)3 twee keer — de audit maakt
hem nog steeds, de worker weigert hem nu (bij A5e.3-veld werd hij geleverd). (3) **De M-T-as levert op alle drie
de posities**, waar A5e.3-veld op 1294 en 1495 niets leverde; de tweeter meet in de worker −20,10 tot −28,83 op
de posities 1647/1948 en −25 tot −26 op 2304. Dat "−20,10" is stap 2. **Welk getal de monitor afdrukt, en welk niet.** De samenvattingsregel van de generator ("2,30 dB / 6,4°") is `net.after.rippleDb` en `.phaseDeg`: de piek-tot-piek-afwijking en de fasemaat van de TUNER zelf, tegen het trapdoel van 2,5 dB — géén RMS. De tabel draagt hem als eigen kolom ("tuner-rimpel dB / fase °") naast de twee RMS-kolommen — de volle oordeelband 52–19 500 Hz vanaf f_p, en vanaf de tweeter-gate op 397 Hz — elk met het ±venster apart. De geleverde A5e.3c-netwerken lezen 2,26–2,43 dB op die tuner-maat en 0,85–1,20 dB RMS op de volle band: exact het bereik van A5e.3-veld (0,85–1,20). Het is een andere kolom, geen verslechtering.

**STAP 2 — DE BEVINDING: TWEE OORDELEN OVER ÉÉN EIS, EN DE AFGEDRUKTE WAS DE MILDSTE (de V32-vorm, op M-C).**
De tabel van `measure-a5e3c-field.ts` (één meetbank, het rapport op het BESTAND) las op drie van de tien bevroren
netlists een falend M-C op de tweeter — −19,90 / −19,76 / −19,61 dB tegen −20,0 — terwijl de worker ze op −20,10 /
−20,14 / −20,19 had doorgelaten. Gediagnosticeerd door het bestand door de poortroute van de worker zelf te halen
(`freezeGateReference` + `evaluateGates`, de V32-constructie): het bestand leest daar EXACT wat het rapport leest
(tot 0,01 dB, op alle tien), dus poort en rapport zijn het eens over het bestand — wat afwijkt is wat de worker
OPNEEMT. `gates.ts` regel 4: de passbands van M-C worden bij de start van een run BEVROREN op het zaad (anders
haalt de optimalisator M-C door het kruispunt te verplaatsen), en het geleverde netwerk wordt daarna ÓÓK op de
passbands van zijn eigen kruispunten beoordeeld — "passing one and failing the other is a finding, and it is
reported as one". Berekend werd het (`gatesDerived`, de gecombineerde `violation`); gelezen werd het op de v2-route
door niemand: de shortlist oordeelt op `gates` (de bevroren helft), de generator schrijft `poorten` daaruit, en de
tuner-hook weigert alleen op de bevroren helft. Het mechanisme is meetbaar en systematisch: de tuner verplaatst het
M-T-kruispunt van de gestelde positie NAAR BENEDEN (1948 → 1720 / 1776 / 1772 Hz; 1647 → 1637–1772; 2304 →
2049–2243), de tweeter-passband wordt naar beneden verbreed tot op zijn eigen dalende flank, het passband-gemiddelde
zakt, en M-C op de tweeter leest tot 0,58 dB MINDER beschermend op de eigen kruispunten dan op die van het zaad (dertien van veertien geleverde; −0,02 tot +0,58)
(op het A5e.3-veld-corpus +0,01 tot +0,33 dB op zes van zeven, dezelfde richting; op de mid −0,1 tot −2,0 dB op dertien van veertien de ANDERE kant op, en daar ligt de afgeleide grens ver weg). Drie van de veertien geleverde netwerken haalden −20 op het zaad met 0,10–0,19 dB
en missen het op zichzelf met 0,10–0,39 dB. **Dat is geen afronding: de stated-figure-vloer van A5e.3b (1647 Hz)
is de inversie van −20 dB bij orde 4 op het GESTELDE kruispunt, dus een positie op of nabij die vloer is een
RAND en geen marge — en de tuner schuift het kruispunt van die rand af.** Een oordeel dat alleen berekend wordt
is geen oordeel (V31): sinds A5e.3c weigert de worker een geleverd netwerk dat op de eigen kruispunten een
actieve poort mist — `by: 'derived-gate'`, `kinds: ['gate']`, de zin van de poort als reden, in de vorm van de
stated-budget/stated-topology-weigeringen (`derivedGateViolation`, één callback voor beide conventies, de bevroren
eerst; een bevroren fout blijft de weigering van de tuner zelf). `runV2Optimization` (`run.ts`) weigerde al op
beide; de workerroute liep achter. De bevroren helft blijft waaraan de ZOEKTOCHT gehouden wordt.

**De reparatie raakt aantoonbaar precies drie van de vierentwintig**, en dat is gemeten en niet beredeneerd
(de A5e.3-veld-vorm van `V2_ONLY` + `V2_MERGE=1`): de weigering vuurt alleen op een GELEVERD netwerk dat op de
eigen kruispunten faalt; de tien geweigerde waren al geweigerd vóór zij leest, en de elf andere geleverde lezen
op hun eigen kruispunten −20,15 tot −28,62 op de tweeter en −17,40 tot −49,65 op de mid tegen −12,1 tot −14,7 —
allemaal binnen. De drie (215,2 · 1948, 313,2 · 1948, 455,7 · 1948 — alle drie op de 1948-positie) zijn opnieuw
gedraaid onder de gerepareerde worker (dezelfde tune, deterministisch; daarna de weigering) en de shards zijn
samengevoegd. De herhaling kostte 2637 / 9797 / 10 192 s (drie tegelijk) en reproduceerde elke tune exact — dezelfde evaluatietelling (304 706 / 767 276 / 691 865), dezelfde spoelen, dezelfde rimpel — met de weigering erachter (A5e.4, per (machine, runtime) en nu ook per engine-stand: de negentien andere shards zijn wat een herhaling zou opleveren, en de drie zijn herhaald in plaats van herschreven). Wat de shortlist daarna kiest (stap 3) is uit ELF geleverde in plaats van veertien.

**Bijvangst, geboekt en niet gerepareerd:** over het HELE casusboek vinden de poortroute (kruispunten op het
ketenraster, 143 punten) en het rapport (1600 punten) op ZESTIEN gedateerde netlists (V28–V50) ANDERE kruispunten
— een mid waarvan het ketenraster de onderste overname op ~490 Hz legt waar het rapportraster ~385 vindt, en op de
twee V28-netlists een M-T-kruispunt op 1521/1613 tegen 3818/3949 — zodat M-C daar tot 5 dB (mid) en 3 dB (tweeter)
uiteenloopt. De M-C-cijfers van het casusboek zijn die van het RAPPORT (klasse B); `derivedGateRefusal.test.ts`
draagt de lijst in de V30-vorm (boekhouding die leeg hoort te raken; een nieuwe afwijking valt door er niet op te
staan). Op het levende corpus, het A5e.3-veld-corpus, de arm en de drie referentiefilters zijn de twee routes het
binnen de dB-klasse eens.

**STAP 3 — DE SHORTLIST-GROOTTE ALS DERDE TOESTAND.** Elf van de vierentwintig kandidaten halen alles (vloer, M-C
per weg op zaad én eigen kruispunten, niveauwerk, M-A/part); de shortlist houdt `DEFAULT_SHORTLIST_SIZE` = 10
ontwerpen en kiest die op spreiding (`selectDiverse`: klasse-sleutel, dan de genormaliseerde componentafstand),
dus 549,7 · 2304 (RMS 1,01 / 0,91, M-K 9,7 / 14,1°, 20 onderdelen, € 222 — het GOEDKOOPSTE geleverde netwerk) is GELEVERD MAAR NIET BEVROREN — geen poort, geen eis, alleen de grootte van de lijst. A5e.3c is de
eerste regeneratie waarin dat voorkomt (elk vorig veld leverde ≤ 10); zulke netwerken bestaan alleen in de shards
van de generator (gitignored), en de tabel draagt ze als eigen rij met de vector in het JSON, zodat de tabel ook
zonder de shards leesbaar blijft. Wie ze wil bekijken vraagt om een grotere `size` — een presentatiekeuze (A5e.1), geen oordeel.

**DE 2304-POSITIE, GEMETEN.** De eerste golf las alsof élke kandidaat op M-T 2304 op de vloer of op de topologie viel
(147,9 en 178,4 op de vloer, 313,2 en 455,7 op het rimpeldoel); over het hele veld hield dat NIET stand — vier van de acht
leverden, drie zijn bevroren. Per 2304-kandidaat: waar het minimum zit, wat het rimpeldoel mist, en waar het geleverde
M-T-kruispunt landde:

| W-M | uitkomst op 2304 | min \|Z\| Ω (tak @ Hz) | tuner-rimpel dB tegen 2,5 | geleverd M-T-kruispunt |
| --- | --- | --- | --- | --- |
| 147,9 | vloer: geweigerde tune 2,53 (poort 2,52) | minimum in een andere weg dan de woofer — Y onoplosbaar met 20 Ω aan de kop van de woofer | 3,29 | — |
| 178,4 | vloer: 2,51 | idem, Y onoplosbaar | 3,10 | — |
| 215,2 | **GELEVERD** (KAND-V2-7) | 2,59 woofer @ 262 | 2,35 | 2243 (−61) |
| 259,6 | **GELEVERD** (KAND-V2-6) | 2,58 woofer @ 294 | 2,26 | 2052 (−252) |
| 313,2 | topologie: rimpel 4,60 (mist 2,10; X = 0,78) | — | 4,60 | — |
| 377,8 | **GELEVERD** (KAND-V2-2) | 2,62 woofer @ 425 | 2,30 | 2049 (−255) |
| 455,7 | topologie: rimpel 4,15 (mist 1,65; X = 0,78) | — | 4,15 | — |
| 549,7 | GELEVERD, niet bevroren | 2,57 woofer @ 51 | 2,31 | 2050 (−254) |

Twee dingen staan erin. Waar 2304 valt, valt hij op de VLOER bij de twee laagste W-M-posities (het minimum in de mid- of
tweetertak, onoplosbaar met serieweerstand op de woofer) en op het RIMPELDOEL in het midden — geen van beide is de
positie zelf. En élk geleverd 2304-netwerk is door de tuner 61–255 Hz naar beneden getrokken (2243, 2052, 2049, 2050):
de gestelde bovenpositie is de start en niet het antwoord, en drie van de vier landen op ~2050 Hz. **Voorstel, niet
gebouwd: de M-T-BOVENGRENS als "strengste van gesteld en afgeleid", het spiegelbeeld van de vloer van A5e.3b** — het
venster leest nu alleen het breakup-plafond; wat de tuner op deze positie doet (~250 Hz drift) en de lobing-zone zijn de
afgeleide helften die er een grens van kunnen maken. Een besluit over de M-T-as, één regeneratie.

**DE VLOERLANDING, ALS CONTEXT VOOR DE 4,0 Ω-VRAAG.** Negen van de elf geleverde netwerken landen op 2,57–2,62 Ω
(2,55 en 2,62 zijn de uitersten): de barrière mikt op de vloer en de poort laat tot 2,548 toe (de 2 %-conventie) —
correct gedrag, en het zegt iets over de vraag van A5e.3-veld stap 7 (de fabrieksopgave 4,0 Ω): de tuner koopt de laatste
tiende ohm met rimpel of fase, dus een hogere gestelde vloer verschuift het HELE veld en niet één kandidaat. Dat is een
arm ná deze regeneratie (de NAD-arm van A5e.3-veld, op dit veld), niet nu.

**LOOPTIJDEN, EN EEN AANBEVELING.** Per kandidaat 1410–14 690 s (kolom in de tabel, uit de shards; de herhaalde drie
dragen hun herhalingstijd: 10 192 / 9797 / 2637 tegen 11 634 / 10 988 / 2828 in de eerste run). De weigeringen zijn
goedkoop (1410–3348 s, zij vallen vroeg); de geleverde met 650 000–900 000 evaluaties zijn de dure: 215,2 · 2304
(14 690), 215,2 · 1948 (11 634), 313,2 · 1948 (10 988), 259,6 · 2304 (10 961), 377,8 · 2304 (9158), 377,8 · 1948 (8671).
**Aanbeveling voor een volgende regeneratie: positiebudget 16, en posities onder 215 Hz alleen als het ontwerp daar echt
heen wil** — wat daar iets oplevert is de duurste zoektocht van het veld (de grote spoelen op de laagste weg, de val op
de cap), en wat er valt, valt op de vloer.

**STAP 4 — DE TABEL PER KANDIDAAT** (`scripts/measure-a5e3c-field.ts`; de volle vector, DCR per spoel en BOM per
onderdeel staan in `casus1_a5e3c_veld_tabel.json`; het gedateerde A5e.3-veld erbij, de A5e.3b-ablatie-arm
"bouwbare val" uit `casus1_a5e3b_ablatie/bouwbaar.json`, en HUIDIG met pad op 360 Hz):
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| A5e.3c | 147.9 · 1646.9 | GEWEIGERD — gate: M-B/|Z|: 2.39 Ω falls below the stated floor of 2.60 Ω | 3.19 / 22.4 | 2488 | 148 · 1647 | geweigerde tune 2.48 (Y —) | geweigerde tune 2.07 / ±73.06 | — | — | geweigerde tune -18.7 | — | — | — | — | 4.46 / 8.26 / 3.07 | 22.00 | — | — | — | — |
| A5e.3c | 147.9 · 1947.9 | GEWEIGERD — gate: M-B/|Z|: 2.23 Ω falls below the stated floor of 2.60 Ω | 3.19 / 18.3 | 3348 | 148 · 1948 | geweigerde tune 2.24 (Y —) | geweigerde tune 3.20 / ±74.64 | — | — | geweigerde tune -19.1 | — | — | — | — | 2.75 / 7.03 / 5.22 | 8.96 | — | — | — | — |
| A5e.3c | 147.9 · 2304 | GEWEIGERD — gate: M-B/|Z|: 2.52 Ω falls below the stated floor of 2.60 Ω | 3.29 / 19.6 | 3262 | 148 · 2304 | geweigerde tune 2.53 (Y —) | geweigerde tune 2.77 / ±74.01 | — | — | geweigerde tune -14.9 | — | — | — | — | 2.82 / 7.27 / 4.06 | 7.62 | — | — | — | — |
| A5e.3c | 178.4 · 1646.9 | GELEVERD | 2.33 / 6.4 | 2171 | 178 · 1647 → 254 · 1637 | 2.58 @ 941 in mid ✓ | 1.20 / ±2.19 | 0.77 / ±1.26 | 6.8 | mid -20.5 (-13.9) / tweeter -20.1 (-20.0) | -2.90 / 2.08 | 1.43 | 24 | 1.18 (5.0) B·R9 | 2.56 / 7.15 / 4.04 | 7.77 | 7.77 mH / 1531 µF @ 46 Hz + R 1.71 (DCR 1.01) | -1.6 | 37 | 354 (11× meervoudig) ONDERGRENS: C6 zonder realisatie |
| A5e.3c | 178.4 · 1947.9 | GEWEIGERD — topology: this configuration asks 0.78 dB of level work on the lowest way (woofer sits that far above the anchor mid after the target curve, A5d.4) and the project forbids level work there; without more the delivered network misses the ripple goal: 2.58 dB peak deviation against 2.5 dB | 2.58 / 15.1 | 1797 | 178 · 1948 | geweigerde tune — (Y 1.20) | geweigerde tune 1.61 / ±71.80 | — | — | geweigerde tune -13.4 | — | — | — | — | 2.70 / 6.00 / 1.76 | 8.39 | — | — | — | — |
| A5e.3c | 178.4 · 2304 | GEWEIGERD — gate: M-B/|Z|: 2.51 Ω falls below the stated floor of 2.60 Ω | 3.10 / 16.5 | 2702 | 178 · 2304 | geweigerde tune 2.51 (Y —) | geweigerde tune 2.80 / ±73.94 | — | — | geweigerde tune -13.0 | — | — | — | — | 3.06 / 6.79 / 1.79 | 12.43 | — | — | — | — |
| A5e.3c | 215.2 · 1646.9 | GEWEIGERD — topology: this configuration asks 0.78 dB of level work on the lowest way (woofer sits that far above the anchor mid after the target curve, A5d.4) and the project forbids level work there; without more the delivered network misses the ripple goal: 3.75 dB peak deviation against 2.5 dB | 3.75 / 15.0 | 1725 | 215 · 1647 | geweigerde tune — (Y 2.07) | geweigerde tune 1.97 / ±70.70 | — | — | geweigerde tune -21.7 | — | — | — | — | 3.46 / 5.34 / 2.67 | 14.70 | — | — | — | — |
| A5e.3c | 215.2 · 1947.9 | GEWEIGERD — gate: on the passbands its own crossings imply — M-C (tweeter): -19.9 dB exceeds the stated ceiling of -20.0 dB | 2.34 / 3.2 | 10192 | 215 · 1948 | geweigerde tune — (Y 1.15) | geweigerde tune 1.09 / ±71.52 | — | — | geweigerde tune -16.9 | — | — | — | — | 4.25 / 8.08 / 2.05 | 17.41 | — | — | — | — |
| A5e.3c | 215.2 · 2304 | GELEVERD | 2.35 / 2.8 | 14690 | 215 · 2304 → 281 · 2243 | 2.59 @ 262 in woofer ✓ | 1.07 / ±2.20 | 0.70 / ±1.45 | 2.3 / 3.1 | mid -19.6 (-14.7) / tweeter -25.9 (-20.0) | -1.74 / 2.01 | 1.25 | 17 | 0.70 (5.0) B·R9 | 4.22 / 8.93 / 1.40 | 17.57 | 17.57 mH / 743 µF @ 44 Hz (DCR 1.62) | -2.9 | 40 | 484 (12× meervoudig) |
| A5e.3c | 259.6 · 1646.9 | GELEVERD | 2.30 / 6.7 | 5526 | 260 · 1647 → 341 · 1772 | 2.59 @ 337 in woofer ✓ | 0.97 / ±2.60 | 0.77 / ±1.66 | 14.9 / 3.9 | mid -24.0 (-13.5) / tweeter -20.3 (-20.0) | -1.15 / 1.95 | 1.58 | 18 | 0.90 (5.0) B·R9 | 4.23 / 1.72 / 0.74 | 22.00 | 22.00 mH / 342 µF @ 58 Hz (DCR 1.85) | -1.8 | 24 | 301 (5× meervoudig) |
| A5e.3c | 259.6 · 1947.9 | GELEVERD | 2.31 / 9.7 | 3059 | 260 · 1948 → 295 · 2043 | 2.58 @ 228 in woofer ✓ | 1.11 / ±2.16 | 0.80 / ±1.41 | 36.8 / 3.1 | mid -31.3 (-14.0) / tweeter -23.6 (-20.0) | -2.41 / 2.04 | 1.43 | 26 | 0.62 (5.0) C·R5 | 2.85 / 9.66 / 1.69 | 18.76 | 18.76 mH / 504 µF @ 52 Hz + R 1.29 (DCR 1.68) | -2.7 | 40 | 411 (10× meervoudig) |
| A5e.3c | 259.6 · 2304 | GELEVERD | 2.26 / 3.3 | 10961 | 260 · 2304 → 341 · 2052 | 2.58 @ 294 in woofer ✓ | 0.99 / ±2.10 | 0.74 / ±1.49 | 2.7 / 3.6 | mid -21.5 (-14.5) / tweeter -23.4 (-20.0) | -0.42 / 2.06 | 1.26 | 16 | 0.72 (5.0) B·R9 | 4.57 / 8.40 / 1.74 | 22.00 | 22.00 mH / 561 µF @ 45 Hz (DCR 1.85) | -2.6 | 38 | 492 (11× meervoudig) |
| A5e.3c | 313.2 · 1646.9 | GELEVERD | 2.43 / 14.0 | 1787 | 313 · 1647 → 341 · 1728 | 2.57 @ 49 in woofer ✓ | 1.10 / ±2.30 | 0.73 / ±1.37 | 5.0 / 18.1 | mid -21.0 (-12.1) / tweeter -20.8 (-20.0) | -2.63 / 2.03 | 1.42 | 23 | 1.61 (5.0) B·R9 | 3.87 / 7.17 / 2.31 | 13.59 | 13.59 mH / 584 µF @ 57 Hz (DCR 1.40) | -2.1 | 36 | 409 (5× meervoudig) ONDERGRENS: R10, C·R12 zonder realisatie |
| A5e.3c | 313.2 · 1947.9 | GEWEIGERD — gate: on the passbands its own crossings imply — M-C (tweeter): -19.8 dB exceeds the stated ceiling of -20.0 dB | 2.32 / 3.4 | 9797 | 313 · 1948 | geweigerde tune — (Y 1.15) | geweigerde tune 1.00 / ±71.34 | — | — | geweigerde tune -20.1 | — | — | — | — | 4.26 / 7.96 / 1.46 | 17.10 | — | — | — | — |
| A5e.3c | 313.2 · 2304 | GEWEIGERD — topology: this configuration asks 0.78 dB of level work on the lowest way (woofer sits that far above the anchor mid after the target curve, A5d.4) and the project forbids level work there; without more the delivered network misses the ripple goal: 4.60 dB peak deviation against 2.5 dB | 4.60 / 8.9 | 2046 | 313 · 2304 | geweigerde tune — (Y 1.16) | geweigerde tune 1.75 / ±73.61 | — | — | geweigerde tune -26.2 | — | — | — | — | 4.29 / 5.53 / 0.99 | 19.07 | — | — | — | — |
| A5e.3c | 377.8 · 1646.9 | GELEVERD | 2.30 / 8.9 | 2280 | 378 · 1647 → 457 · 1699 | 2.59 @ 52 in woofer ✓ | 0.92 / ±2.58 | 0.78 / ±2.09 | 7.0 / 16.1 | mid -25.2 (-14.0) / tweeter -28.6 (-20.0) | -1.16 / 1.86 | 1.53 | 17 | 1.70 (5.0) B·R9 | 2.89 / 1.58 / 1.96 | 22.00 | 22.00 mH / 347 µF @ 58 Hz (DCR 1.85) | -1.7 | 20 | 237 (2× meervoudig) |
| A5e.3c | 377.8 · 1947.9 | GELEVERD | 2.28 / 4.2 | 8671 | 378 · 1948 → 432 · 1717 | 2.55 @ 415 in woofer ✓ | 0.85 / ±2.06 | 0.78 / ±1.98 | 4.9 / 3.8 | mid -29.6 (-13.9) / tweeter -23.7 (-20.0) | -0.99 / 1.89 | 1.49 | 21 | 1.16 (5.0) B·R9 | 4.35 / 9.70 / 1.83 | 22.00 | 22.00 mH / 360 µF @ 57 Hz (DCR 1.85) | -1.9 | 37 | 456 (8× meervoudig) |
| A5e.3c | 377.8 · 2304 | GELEVERD | 2.30 / 4.1 | 9158 | 378 · 2304 → 437 · 2049 | 2.62 @ 425 in woofer ✓ | 0.86 / ±2.19 | 0.80 / ±2.07 | 3.2 / 4.5 | mid -30.4 (-14.3) / tweeter -24.9 (-20.0) | -1.10 / 1.84 | 1.47 | 19 | 1.14 (5.0) B·R9 | 4.36 / 6.68 / 2.61 | 22.00 | 22.00 mH / 359 µF @ 57 Hz (DCR 1.85) | -2.7 | 39 | 440 (9× meervoudig) |
| A5e.3c | 455.7 · 1646.9 | GEWEIGERD — topology: woofer carries a shunt chain without a resonance (L5 16.50 mH + R7 24.33 Ω): no capacitor, so it traps nothing — a load, not a filter element, and the rule forbids every pad (A5e.3b) (lowestWayLevelWork: none, V51 — refused since A5e.3-veld) | 2.30 / 8.3 | 6039 | 456 · 1647 | geweigerde tune — (Y 0.85) | geweigerde tune 1.15 / ±71.52 | — | — | geweigerde tune -11.3 | — | — | — | — | 4.26 / 5.39 / 4.95 | 22.00 | — | — | — | — |
| A5e.3c | 455.7 · 1947.9 | GEWEIGERD — gate: on the passbands its own crossings imply — M-C (tweeter): -19.6 dB exceeds the stated ceiling of -20.0 dB | 2.31 / 12.6 | 2637 | 456 · 1948 | geweigerde tune — (Y 0.92) | geweigerde tune 1.13 / ±71.59 | — | — | geweigerde tune -12.7 | — | — | — | — | 2.18 / 5.35 / 2.52 | 11.36 | — | — | — | — |
| A5e.3c | 455.7 · 2304 | GEWEIGERD — topology: this configuration asks 0.78 dB of level work on the lowest way (woofer sits that far above the anchor mid after the target curve, A5d.4) and the project forbids level work there; without more the delivered network misses the ripple goal: 4.15 dB peak deviation against 2.5 dB | 4.15 / 9.9 | 1410 | 456 · 2304 | geweigerde tune — (Y 0.88) | geweigerde tune 1.69 / ±73.31 | — | — | geweigerde tune -34.6 | — | — | — | — | 2.73 / 4.84 / 1.62 | 21.99 | — | — | — | — |
| A5e.3c | 549.7 · 1646.9 | GELEVERD | 2.31 / 4.5 | 6316 | 550 · 1647 → 553 · 1704 | 2.59 @ 607 in mid ✓ | 0.96 / ±2.45 | 0.77 / ±1.80 | 5.7 / 3.8 | mid -49.7 (-12.9) / tweeter -26.0 (-20.0) | -1.29 / 1.61 | 1.40 | 25 | 1.80 (5.0) B·R9 | 2.72 / 6.39 / 1.80 | 22.00 | 22.00 mH / 355 µF @ 57 Hz + R 0.28 (DCR 1.85) | -2.1 | 32 | 314 (7× meervoudig) |
| A5e.3c | 549.7 · 1947.9 | GEWEIGERD — topology: woofer carries a shunt chain without a resonance (L8 11.41 mH + R10 1.29 Ω): no capacitor, so it traps nothing — a load, not a filter element, and the rule forbids every pad (A5e.3b) (lowestWayLevelWork: none, V51 — refused since A5e.3-veld) | 2.30 / 4.7 | 3101 | 550 · 1948 | geweigerde tune — (Y 0.81) | geweigerde tune 0.79 / ±71.44 | — | — | geweigerde tune -25.6 | — | — | — | — | 3.92 / 3.31 / 1.19 | 22.00 | — | — | — | — |
| A5e.3c (geleverd, niet bevroren) | 549.7 · 2304 | GELEVERD, NIET BEVROREN — de shortlist houdt 10 ontwerpen en koos op spreiding (selectDiverse); geen poort en geen eis | 2.31 / 11.5 | 2042 | 550 · 2304 → 535 · 2050 | 2.57 @ 51 in woofer ✓ | 1.01 / ±2.19 | 0.91 / ±2.19 | 9.7 / 14.1 | mid -32.2 (-14.4) / tweeter -24.9 (-20.0) | -1.12 / 1.64 | 1.35 | 14 | 1.33 (5.0) B·R9 | 2.75 / 1.41 / 1.16 | 22.00 | 22.00 mH / 385 µF @ 55 Hz (DCR 1.85) | -2.5 | 20 | 222 (5× meervoudig) |
| A5e.3-veld | 549.7 · 1494.8 | GELEVERD | — | — | 550 · 1495 → 409 · 2303 | 2.81 @ 322 in woofer ✓ | 0.85 / ±2.33 | 0.84 / ±2.02 | 13.4 / 5.8 | tweeter -21.2 (-20.0) | -1.32 / 1.86 | 1.49 | 9 | 0.31 (5.0) C·R9 | 4.43 / 4.35 / 1.27 | 22.63 > span | 22.63 mH / 342 µF @ 57 Hz + R 1.26 (DCR 1.88) | -3.2 | 28 | 364 (5× meervoudig) ONDERGRENS: C·R5 zonder realisatie |
| A5e.3-veld | 229.1 · 1726.7 | GELEVERD | — | — | 229 · 1727 → 358 · 1558 | 2.85 @ 25 in woofer ✓ | 0.90 / ±2.03 | 0.68 / ±1.54 | 3.0 / 6.8 | mid -19.7 (-13.6) / tweeter -21.3 (-20.0) | -0.49 / 1.29 | 1.26 | 24 | 1.06 (5.0) B·R9 | 1.88 / 8.62 / 1.20 | 5.39 | geen | -1.6 | 32 | 287 (8× meervoudig) |
| A5e.3-veld | 229.1 · 2304 | GELEVERD | — | — | 229 · 2304 → 339 · 2049 | 2.59 @ 311 in woofer ✓ | 0.97 / ±2.09 | 0.66 / ±1.44 | 7.6 / 3.5 | mid -31.7 (-14.3) / tweeter -24.6 (-20.0) | -0.64 / 2.06 | 1.34 | 22 | 1.18 (5.0) B·R9 | 4.64 / 7.44 / 0.93 | 21.95 | 21.95 mH / 516 µF @ 47 Hz + R 1.00 (DCR 1.84) | -2.6 | 35 | 458 (8× meervoudig) ONDERGRENS: C·R5 zonder realisatie |
| A5e.3-veld | 549.7 · 1994.6 | GELEVERD | — | — | 550 · 1995 → 553 · 1785 | 2.60 @ 975 in mid ✓ | 0.97 / ±2.47 | 0.77 / ±1.88 | 5.2 / 4.0 | mid -46.3 (-13.4) / tweeter -24.5 (-20.0) | -1.11 / 1.61 | 1.40 | 24 | 1.93 (5.0) B·R9 | 2.82 / 5.43 / 1.85 | 24.01 > span | 24.01 mH / 328 µF @ 57 Hz + R 0.23 (DCR 1.94) | -2.0 | 31 | 320 (7× meervoudig) |
| A5e.3-veld | 354.9 · 1994.6 | GELEVERD | — | — | 355 · 1995 → 479 · 1974 | 2.70 @ 53 in woofer ✓ | 1.03 / ±2.50 | 0.85 / ±2.11 | 3.8 / 5.8 | mid -30.3 (-13.9) / tweeter -24.0 (-20.0) | -0.93 / 1.70 | 1.46 | 14 | 1.16 (5.0) B·R9 | 3.06 / 1.53 / 1.13 | 27.85 > span | 27.85 mH / 280 µF @ 57 Hz (DCR 2.12) | -2.4 | 18 | 267 (6× meervoudig) |
| A5e.3-veld | 549.7 · 1726.7 | GELEVERD | — | — | 550 · 1727 → 526 · 1942 | 2.66 @ 53 in woofer ✓ | 1.17 / ±2.29 | 1.18 / ±2.15 | 22.0 / 11.3 | mid -27.1 (-14.3) / tweeter -25.0 (-20.0) | -1.90 / 1.65 | 1.37 | 12 | 0.57 (5.0) R7 | 2.05 / 4.48 / 3.29 | 9.65 | 9.65 mH / 733 µF @ 60 Hz + R 0.94 (DCR 1.15) | -2.0 | 28 | 277 (8× meervoudig) ONDERGRENS: C·R12 zonder realisatie |
| A5e.3-veld | 354.9 · 1494.8 | GELEVERD | — | — | 355 · 1495 → 429 · 1594 | 2.60 @ 13989 in tweeter ✓ | 1.20 / ±2.34 | 1.08 / ±2.14 | 5.2 / 12.7 | mid -39.2 (-14.4) / tweeter -20.4 (-20.0) | -0.61 / 1.67 | 1.47 | 10 | 0.43 (5.0) C·R9 | 4.96 / 4.76 / 0.97 | 35.61 > span | 35.61 mH / 221 µF @ 57 Hz (DCR 2.44) | -1.8 | 28 | 460 (7× meervoudig) |
| A5e.3b-arm | A5e.3b-arm bouwbare val (354,9 · 1994,6, L ≤ 22 mH) | GELEVERD | — | — | — → 455 · 2102 | 2.59 @ 51 in woofer ✓ | 0.86 / ±1.93 | 0.73 / ±1.70 | 3.9 / 4.7 | mid -28.3 (-14.3) / tweeter -27.5 (-20.0) | -1.48 / 1.70 | 1.37 | 13 | 0.92 (5.0) B·R9 | 2.78 / 1.45 / 0.76 | 22.00 | 22.00 mH / 377 µF @ 55 Hz (DCR 1.85) | -2.7 | 18 | 212 (4× meervoudig) |
| HUIDIG (met pad) | HUIDIG | GELEVERD | — | — | — → 360 · 2250 | 3.46 @ 218 in woofer ✓ | 1.42 / ±3.66 | 0.58 / ±1.17 | 21.0 / 7.0 | mid -42.5 (-11.8) / tweeter -25.1 (-20.0) | -0.94 / 4.69 | 2.31 | 46 | 2.55 (5.0) R8 | 0.74 / 1.80 / 1.24 | 3.00 | geen | -2.9 | 23 | 142 (2× meervoudig) |

  Gepaard tegen A5e.3-veld — op LABEL waar het kan (nul van elf: de M-T-as verhuisde en de W-M-as verdubbelde, `corpusPairing` zegt n = 0), anders het dichtstbijzijnde kruispunt met de octaafafstand erbij (anekdote per rij, geen corpusdelta):

| A5e.3c | A5e.3-veld (afstand oct) | min \|Z\| Ω | RMS volle band dB | RMS vanaf 397 dB | M-K W-M / M-T ° | M-C tweeter dB | opslingering dB | dissipatie % | heetste R W bij 10 W | koper woofer Ω | onderdelen | BOM € | lobing dip dB |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 178.4 · 1646.9 | 229.1 · 1726.7 (0.43) | 2.85 → 2.58 (-0.27) | 0.90 → 1.20 (+0.30) | 0.68 → 0.77 (+0.09) | 3.00 → 6.76 (+3.76) | -21.29 → -20.15 (+1.14) | -0.49 → -2.90 (-2.41) | 23.81 → 23.65 (-0.16) | 1.06 → 1.18 (+0.11) | 1.88 → 2.56 (+0.68) | 32.00 → 37.00 (+5.00) | 287.37 → 353.64 (+66.27) | -1.57 → -1.64 (-0.07) |
| 215.2 · 2304 | 229.1 · 2304 (0.09) | 2.59 → 2.59 (+0.00) | 0.97 → 1.07 (+0.11) | 0.66 → 0.70 (+0.05) | 7.59 → 2.28 (-5.31) / 3.52 → 3.14 (-0.37) | -24.55 → -25.91 (-1.36) | -0.64 → -1.74 (-1.10) | 21.56 → 17.06 (-4.51) | 1.18 → 0.70 (-0.48) | 4.64 → 4.22 (-0.42) | 35.00 → 40.00 (+5.00) | 457.93 → 483.80 (+25.87) | -2.60 → -2.94 (-0.34) |
| 259.6 · 1646.9 | 229.1 · 1726.7 (0.25) | 2.85 → 2.59 (-0.26) | 0.90 → 0.97 (+0.07) | 0.68 → 0.77 (+0.09) | 3.00 → 14.92 (+11.92) / 6.80 → 3.91 (-2.88) | -21.29 → -20.35 (+0.94) | -0.49 → -1.15 (-0.66) | 23.81 → 18.40 (-5.41) | 1.06 → 0.90 (-0.16) | 1.88 → 4.23 (+2.35) | 32.00 → 24.00 (-8.00) | 287.37 → 300.97 (+13.60) | -1.57 → -1.84 (-0.27) |
| 259.6 · 1947.9 | 229.1 · 1726.7 (0.35) | 2.85 → 2.58 (-0.27) | 0.90 → 1.11 (+0.21) | 0.68 → 0.80 (+0.12) | 3.00 → 36.84 (+33.83) / 6.80 → 3.15 (-3.65) | -21.29 → -23.63 (-2.35) | -0.49 → -2.41 (-1.92) | 23.81 → 26.21 (+2.40) | 1.06 → 0.62 (-0.44) | 1.88 → 2.85 (+0.97) | 32.00 → 40.00 (+8.00) | 287.37 → 410.85 (+123.48) | -1.57 → -2.70 (-1.12) |
| 259.6 · 2304 | 229.1 · 2304 (0.18) | 2.59 → 2.58 (-0.01) | 0.97 → 0.99 (+0.03) | 0.66 → 0.74 (+0.08) | 7.59 → 2.72 (-4.88) / 3.52 → 3.63 (+0.12) | -24.55 → -23.37 (+1.18) | -0.64 → -0.42 (+0.22) | 21.56 → 15.82 (-5.74) | 1.18 → 0.72 (-0.46) | 4.64 → 4.57 (-0.07) | 35.00 → 38.00 (+3.00) | 457.93 → 492.12 (+34.19) | -2.60 → -2.60 (-0.00) |
| 313.2 · 1646.9 | 354.9 · 1494.8 (0.32) | 2.60 → 2.57 (-0.03) | 1.20 → 1.10 (-0.10) | 1.08 → 0.73 (-0.35) | 5.19 → 5.02 (-0.17) / 12.71 → 18.09 (+5.38) | -20.37 → -20.81 (-0.45) | -0.61 → -2.63 (-2.02) | 9.51 → 22.78 (+13.27) | 0.43 → 1.61 (+1.18) | 4.96 → 3.87 (-1.09) | 28.00 → 36.00 (+8.00) | 459.78 → 409.08 (-50.70) | -1.83 → -2.13 (-0.30) |
| 377.8 · 1646.9 | 354.9 · 1494.8 (0.23) | 2.60 → 2.59 (-0.01) | 1.20 → 0.92 (-0.28) | 1.08 → 0.78 (-0.30) | 5.19 → 6.98 (+1.80) / 12.71 → 16.07 (+3.36) | -20.37 → -28.62 (-8.25) | -0.61 → -1.16 (-0.55) | 9.51 → 17.28 (+7.77) | 0.43 → 1.70 (+1.27) | 4.96 → 2.89 (-2.07) | 28.00 → 20.00 (-8.00) | 459.78 → 236.76 (-223.02) | -1.83 → -1.69 (+0.14) |
| 377.8 · 1947.9 | 354.9 · 1994.6 (0.12) | 2.70 → 2.55 (-0.14) | 1.03 → 0.85 (-0.18) | 0.85 → 0.78 (-0.07) | 3.84 → 4.89 (+1.05) / 5.83 → 3.78 (-2.05) | -23.97 → -23.67 (+0.30) | -0.93 → -0.99 (-0.06) | 13.80 → 20.71 (+6.91) | 1.16 → 1.16 (+0.00) | 3.06 → 4.35 (+1.30) | 18.00 → 37.00 (+19.00) | 267.46 → 455.75 (+188.29) | -2.42 → -1.86 (+0.55) |
| 377.8 · 2304 | 354.9 · 1994.6 (0.30) | 2.70 → 2.62 (-0.07) | 1.03 → 0.86 (-0.17) | 0.85 → 0.80 (-0.05) | 3.84 → 3.17 (-0.66) / 5.83 → 4.49 (-1.34) | -23.97 → -24.85 (-0.88) | -0.93 → -1.10 (-0.17) | 13.80 → 18.56 (+4.76) | 1.16 → 1.14 (-0.02) | 3.06 → 4.36 (+1.31) | 18.00 → 39.00 (+21.00) | 267.46 → 439.70 (+172.24) | -2.42 → -2.68 (-0.26) |
| 549.7 · 1646.9 | 549.7 · 1726.7 (0.07) | 2.66 → 2.59 (-0.07) | 1.17 → 0.96 (-0.21) | 1.18 → 0.77 (-0.41) | 21.96 → 5.66 (-16.31) / 11.34 → 3.82 (-7.52) | -25.03 → -26.03 (-1.00) | -1.90 → -1.29 (+0.61) | 11.87 → 24.61 (+12.74) | 0.57 → 1.80 (+1.23) | 2.05 → 2.72 (+0.68) | 28.00 → 32.00 (+4.00) | 276.66 → 313.89 (+37.23) | -2.04 → -2.09 (-0.05) |
| 549.7 · 2304 | 549.7 · 1994.6 (0.21) | 2.60 → 2.57 (-0.03) | 0.97 → 1.01 (+0.04) | 0.77 → 0.91 (+0.14) | 5.23 → 9.67 (+4.44) / 3.97 → 14.08 (+10.11) | -24.47 → -24.86 (-0.39) | -1.11 → -1.12 (-0.00) | 23.63 → 13.91 (-9.72) | 1.93 → 1.33 (-0.60) | 2.82 → 2.75 (-0.07) | 31.00 → 20.00 (-11.00) | 319.71 → 222.26 (-97.45) | -2.00 → -2.46 (-0.46) |

  De DCR per spoel van de elf geleverde netwerken en de arm (weg, L, gedragen DCR — elke waarde is de fit van de gestelde familie bij de geschreven inductie; op geen enkele spoel staat nog BUITEN BEREIK):

- A5e.3c · 178.4 · 1646.9: L1 (woofer) 3.16 mH → 0.599 Ω; L3 (woofer) 3.16 mH → 0.599 Ω; L5 (woofer) 7.77 mH → 1.010 Ω; L8 (woofer) 1.26 mH → 0.350 Ω; B·L2 (mid) 2.08 mH → 0.804 Ω; B·L4 (mid) 16.32 mH → 2.707 Ω; B·L5 (mid) 0.89 mH → 0.487 Ω; B·L7 (mid) 0.45 mH → 0.325 Ω; B·L11 (mid) 0.97 mH → 0.514 Ω; B·L14 (mid) 12.51 mH → 2.314 Ω; C·L2 (tweeter) 2.93 mH → 0.985 Ω; C·L4 (tweeter) 14.50 mH → 2.525 Ω; C·L7 (tweeter) 0.59 mH → 0.383 Ω; C·L10 (tweeter) 0.11 mH → 0.143 Ω
- A5e.3c · 215.2 · 2304: L1 (woofer) 3.10 mH → 0.592 Ω; L3 (woofer) 2.77 mH → 0.555 Ω; L5 (woofer) 17.57 mH → 1.621 Ω; L8 (woofer) 14.54 mH → 1.452 Ω; B·L2 (mid) 2.05 mH → 0.798 Ω; B·L4 (mid) 12.62 mH → 2.326 Ω; B·L5 (mid) 0.84 mH → 0.473 Ω; B·L7 (mid) 0.28 mH → 0.246 Ω; B·L11 (mid) 14.05 mH → 2.478 Ω; B·L14 (mid) 15.34 mH → 2.610 Ω; C·L2 (tweeter) 0.25 mH → 0.232 Ω; C·L4 (tweeter) 1.46 mH → 0.654 Ω; C·L9 (tweeter) 0.06 mH → 0.100 Ω; C·L15 (tweeter) 0.67 mH → 0.413 Ω
- A5e.3c · 259.6 · 1646.9: L1 (woofer) 3.00 mH → 0.581 Ω; L3 (woofer) 2.50 mH → 0.522 Ω; L5 (woofer) 22.00 mH → 1.847 Ω; L8 (woofer) 11.76 mH → 1.284 Ω; B·L2 (mid) 2.01 mH → 0.789 Ω; B·L5 (mid) 1.36 mH → 0.626 Ω; B·L7 (mid) 0.40 mH → 0.303 Ω; C·L4 (tweeter) 0.67 mH → 0.412 Ω; C·L9 (tweeter) 0.24 mH → 0.228 Ω; C·L12 (tweeter) 0.06 mH → 0.102 Ω
- A5e.3c · 259.6 · 1947.9: L1 (woofer) 3.02 mH → 0.583 Ω; L3 (woofer) 3.02 mH → 0.583 Ω; L5 (woofer) 18.76 mH → 1.684 Ω; B·L2 (mid) 1.96 mH → 0.777 Ω; B·L4 (mid) 16.91 mH → 2.764 Ω; B·L5 (mid) 1.11 mH → 0.556 Ω; B·L7 (mid) 0.28 mH → 0.246 Ω; B·L11 (mid) 15.04 mH → 2.580 Ω; B·L14 (mid) 16.65 mH → 2.739 Ω; C·L2 (tweeter) 0.37 mH → 0.289 Ω; C·L4 (tweeter) 0.64 mH → 0.403 Ω; C·L9 (tweeter) 1.74 mH → 0.724 Ω; C·L12 (tweeter) 0.07 mH → 0.107 Ω; C·L15 (tweeter) 0.14 mH → 0.164 Ω
- A5e.3c · 259.6 · 2304: L1 (woofer) 3.10 mH → 0.592 Ω; L3 (woofer) 3.10 mH → 0.592 Ω; L5 (woofer) 22.00 mH → 1.847 Ω; L8 (woofer) 16.06 mH → 1.539 Ω; B·L2 (mid) 2.24 mH → 0.840 Ω; B·L4 (mid) 13.57 mH → 2.428 Ω; B·L5 (mid) 0.94 mH → 0.503 Ω; B·L7 (mid) 0.29 mH → 0.253 Ω; B·L11 (mid) 12.65 mH → 2.330 Ω; B·L14 (mid) 10.14 mH → 2.045 Ω; C·L2 (tweeter) 0.49 mH → 0.343 Ω; C·L4 (tweeter) 2.84 mH → 0.966 Ω; C·L9 (tweeter) 0.06 mH → 0.098 Ω; C·L12 (tweeter) 0.47 mH → 0.335 Ω
- A5e.3c · 313.2 · 1646.9: L1 (woofer) 2.99 mH → 0.580 Ω; L3 (woofer) 2.99 mH → 0.580 Ω; L5 (woofer) 13.59 mH → 1.396 Ω; L8 (woofer) 12.24 mH → 1.314 Ω; B·L2 (mid) 1.75 mH → 0.727 Ω; B·L4 (mid) 13.90 mH → 2.463 Ω; B·L5 (mid) 1.12 mH → 0.558 Ω; B·L7 (mid) 0.48 mH → 0.337 Ω; B·L11 (mid) 2.05 mH → 0.798 Ω; B·L14 (mid) 12.23 mH → 2.284 Ω; C·L2 (tweeter) 2.11 mH → 0.811 Ω; C·L4 (tweeter) 2.83 mH → 0.965 Ω; C·L7 (tweeter) 0.16 mH → 0.178 Ω; C·L10 (tweeter) 0.51 mH → 0.351 Ω
- A5e.3c · 377.8 · 1646.9: L1 (woofer) 2.85 mH → 0.564 Ω; L3 (woofer) 2.16 mH → 0.480 Ω; L5 (woofer) 22.00 mH → 1.847 Ω; B·L2 (mid) 1.44 mH → 0.649 Ω; B·L5 (mid) 1.11 mH → 0.556 Ω; B·L7 (mid) 0.57 mH → 0.377 Ω; C·L4 (tweeter) 1.49 mH → 0.661 Ω; C·L7 (tweeter) 1.53 mH → 0.670 Ω; C·L10 (tweeter) 1.37 mH → 0.628 Ω
- A5e.3c · 377.8 · 1947.9: L1 (woofer) 2.90 mH → 0.570 Ω; L3 (woofer) 2.29 mH → 0.497 Ω; L5 (woofer) 22.00 mH → 1.847 Ω; L8 (woofer) 14.35 mH → 1.441 Ω; B·L2 (mid) 1.77 mH → 0.731 Ω; B·L4 (mid) 15.29 mH → 2.605 Ω; B·L5 (mid) 1.34 mH → 0.623 Ω; B·L7 (mid) 0.41 mH → 0.311 Ω; B·L11 (mid) 15.10 mH → 2.586 Ω; B·L14 (mid) 17.75 mH → 2.844 Ω; C·L2 (tweeter) 2.01 mH → 0.788 Ω; C·L4 (tweeter) 1.00 mH → 0.522 Ω; C·L7 (tweeter) 0.17 mH → 0.181 Ω; C·L10 (tweeter) 0.47 mH → 0.334 Ω
- A5e.3c · 377.8 · 2304: L1 (woofer) 2.90 mH → 0.570 Ω; L3 (woofer) 2.01 mH → 0.460 Ω; L5 (woofer) 22.00 mH → 1.847 Ω; L8 (woofer) 15.11 mH → 1.485 Ω; B·L2 (mid) 1.67 mH → 0.706 Ω; B·L4 (mid) 14.52 mH → 2.527 Ω; B·L5 (mid) 1.00 mH → 0.524 Ω; B·L7 (mid) 0.33 mH → 0.271 Ω; B·L11 (mid) 7.15 mH → 1.665 Ω; B·L14 (mid) 2.94 mH → 0.986 Ω; C·L2 (tweeter) 0.31 mH → 0.264 Ω; C·L4 (tweeter) 4.00 mH → 1.182 Ω; C·L9 (tweeter) 0.15 mH → 0.172 Ω; C·L12 (tweeter) 2.96 mH → 0.991 Ω
- A5e.3c · 549.7 · 1646.9: L1 (woofer) 2.72 mH → 0.548 Ω; L3 (woofer) 1.13 mH → 0.329 Ω; L5 (woofer) 22.00 mH → 1.847 Ω; B·L2 (mid) 1.16 mH → 0.569 Ω; B·L4 (mid) 11.00 mH → 2.146 Ω; B·L5 (mid) 1.66 mH → 0.705 Ω; B·L7 (mid) 0.59 mH → 0.383 Ω; B·L11 (mid) 15.13 mH → 2.589 Ω; C·L2 (tweeter) 2.23 mH → 0.839 Ω; C·L4 (tweeter) 0.58 mH → 0.379 Ω; C·L9 (tweeter) 0.07 mH → 0.105 Ω; C·L12 (tweeter) 0.86 mH → 0.480 Ω
- A5e.3c (geleverd, niet bevroren) · 549.7 · 2304: L1 (woofer) 2.71 mH → 0.548 Ω; L3 (woofer) 1.28 mH → 0.354 Ω; L5 (woofer) 22.00 mH → 1.847 Ω; B·L2 (mid) 1.13 mH → 0.561 Ω; B·L5 (mid) 1.16 mH → 0.569 Ω; B·L7 (mid) 0.35 mH → 0.284 Ω; C·L4 (tweeter) 0.69 mH → 0.419 Ω; C·L7 (tweeter) 0.50 mH → 0.348 Ω; C·L10 (tweeter) 0.61 mH → 0.391 Ω
- A5e.3b-arm · A5e.3b-arm bouwbare val (354,9 · 1994,6, L ≤ 22 mH): L1 (woofer) 2.51 mH → 0.524 Ω; L3 (woofer) 1.66 mH → 0.412 Ω; L5 (woofer) 22.00 mH → 1.847 Ω; B·L2 (mid) 1.51 mH → 0.667 Ω; B·L5 (mid) 1.10 mH → 0.554 Ω; B·L7 (mid) 0.25 mH → 0.234 Ω; C·L2 (tweeter) 0.25 mH → 0.229 Ω; C·L7 (tweeter) 0.70 mH → 0.425 Ω; C·L10 (tweeter) 0.07 mH → 0.106 Ω

  De volle vector per geleverd netwerk:

- A5e.3c · 178.4 · 1646.9: G 2.83 V, L1 3.163 mH, C2 374.7 uF, L3 3.163 mH, C4 140.5 uF, L5 7.773 mH, C6 1531 uF, R7 1.713 Ω, L8 1.257 mH, C9 65.74 uF, D, B·C1 118.1 uF, B·L2 2.079 mH, B·C3 192 uF, B·L4 16.32 mH, B·L5 0.8872 mH, B·C6 30.44 uF, B·L7 0.4453 mH, B·C8 8.189 uF, B·R9 1.699 Ω, B·L11 0.9713 mH, B·C12 11.08 uF, B·R13 4.224 Ω, B·L14 12.51 mH, B·C15 126 uF, B·R16 2.974 Ω, B·D, C·C1 6.195 uF, C·L2 2.93 mH, C·C3 23.44 uF, C·L4 14.5 mH, C·R5 1.029 Ω, C·R6 44.76 Ω, C·L7 0.5902 mH, C·C8 77.13 uF, C·R9 0.2658 Ω, C·L10 0.1109 mH, C·C11 3.619 uF, C·R12 6.93 Ω, C·D, C10 116.7 uF
- A5e.3c · 215.2 · 2304: G 2.83 V, L1 3.095 mH, C2 256.5 uF, L3 2.772 mH, C4 30.17 uF, L5 17.57 mH, C6 743 uF, L8 14.54 mH, C9 22.26 uF, R10 3.544 Ω, D, B·C1 91.89 uF, B·L2 2.051 mH, B·C3 133.8 uF, B·L4 12.62 mH, B·L5 0.8434 mH, B·C6 17.16 uF, B·L7 0.2786 mH, B·C8 2.875 uF, B·R9 1.012 Ω, B·C10 23.63 uF, B·L11 14.05 mH, B·C12 0.5282 uF, B·R13 21.62 Ω, B·L14 15.34 mH, B·C15 41.74 uF, B·R16 6.403 Ω, B·D, C·C1 8.253 uF, C·L2 0.2519 mH, C·C3 31 uF, C·L4 1.463 mH, C·R5 0.9636 Ω, C·R6 43.99 Ω, C·R7 0.9974 Ω, C·C8 12.07 uF, C·L9 0.06015 mH, C·C10 37.46 uF, C·R11 3.329 Ω, C·L15 0.6712 mH, C·C16 22 uF, C·R17 1.605 Ω, C·D, C10 99.18 uF
- A5e.3c · 259.6 · 1646.9: G 2.83 V, L1 3.003 mH, C2 181.7 uF, L3 2.495 mH, L5 22 mH, C6 341.7 uF, L8 11.76 mH, C9 18.4 uF, D, B·C1 60.58 uF, B·L2 2.01 mH, B·C3 111.1 uF, B·L5 1.359 mH, B·C6 16.83 uF, B·L7 0.3963 mH, B·C8 3.192 uF, B·R9 1.782 Ω, B·D, C·C1 8.466 uF, C·L4 0.6673 mH, C·R5 0.558 Ω, C·R7 8.364 Ω, C·C8 17.32 uF, C·L9 0.2445 mH, C·L12 0.06183 mH, C·C13 17.72 uF, C·R14 4.509 Ω, C·D
- A5e.3c · 259.6 · 1947.9: G 2.83 V, L1 3.018 mH, C2 261.9 uF, L3 3.018 mH, C4 35.11 uF, L5 18.76 mH, C6 503.9 uF, R7 1.294 Ω, C9 26.27 uF, D, B·C1 78.94 uF, B·L2 1.961 mH, B·C3 59.43 uF, B·L4 16.91 mH, B·L5 1.11 mH, B·C6 15.1 uF, B·L7 0.2782 mH, B·C8 2.53 uF, B·R9 1.068 Ω, B·C10 30.2 uF, B·L11 15.04 mH, B·C12 2.189 uF, B·R13 8.223 Ω, B·L14 16.65 mH, B·C15 102.4 uF, B·D, C·C1 18.09 uF, C·L2 0.3655 mH, C·C3 20 uF, C·L4 0.6423 mH, C·R5 0.9801 Ω, C·R6 44.19 Ω, C·R7 6.381 Ω, C·C8 23.54 uF, C·L9 1.738 mH, C·C10 10.69 uF, C·L12 0.06755 mH, C·C13 14.64 uF, C·R14 3.873 Ω, C·L15 0.1397 mH, C·C16 100.1 uF, C·R17 0.5741 Ω, C·D, C10 7.749 uF
- A5e.3c · 259.6 · 2304: G 2.83 V, L1 3.096 mH, C2 171.9 uF, L3 3.096 mH, L5 22 mH, C6 561 uF, L8 16.06 mH, C9 13.86 uF, R10 2.287 Ω, D, B·C1 74.96 uF, B·L2 2.238 mH, B·C3 109.6 uF, B·L4 13.57 mH, B·L5 0.9364 mH, B·C6 17.56 uF, B·L7 0.2915 mH, B·C8 3.295 uF, B·R9 0.9994 Ω, B·L11 12.65 mH, B·C12 0.6125 uF, B·R13 21.98 Ω, B·L14 10.14 mH, B·C15 36.55 uF, B·R16 2.406 Ω, B·D, C·C1 7.823 uF, C·L2 0.4891 mH, C·C3 25.31 uF, C·L4 2.836 mH, C·R5 0.6694 Ω, C·R6 46.92 Ω, C·R7 5.83 Ω, C·C8 23.94 uF, C·L9 0.05836 mH, C·C10 73.64 uF, C·R11 6.154 Ω, C·L12 0.4689 mH, C·C13 27 uF, C·R14 0.7683 Ω, C·D, C10 99.99 uF
- A5e.3c · 313.2 · 1646.9: G 2.83 V, L1 2.993 mH, C2 181.9 uF, L3 2.993 mH, C4 47.43 uF, L5 13.59 mH, C6 583.6 uF, L8 12.24 mH, C9 17.88 uF, R10 0.357 Ω, D, B·C1 67.48 uF, B·L2 1.752 mH, B·C3 121.8 uF, B·L4 13.9 mH, B·L5 1.117 mH, B·C6 27.36 uF, B·L7 0.4755 mH, B·C8 2.884 uF, B·R9 2.731 Ω, B·L11 2.051 mH, B·C12 11.45 uF, B·R13 3.905 Ω, B·L14 12.23 mH, B·C15 29.31 uF, B·R16 5.134 Ω, B·D, C·C1 4.914 uF, C·L2 2.109 mH, C·C3 31 uF, C·L4 2.834 mH, C·R5 0.804 Ω, C·R6 67.8 Ω, C·L7 0.1608 mH, C·C8 5.742 uF, C·R9 7.229 Ω, C·L10 0.5077 mH, C·C11 99.88 uF, C·R12 0.2377 Ω, C·D
- A5e.3c · 377.8 · 1646.9: G 2.83 V, L1 2.854 mH, C2 144.7 uF, L3 2.159 mH, C4 51.41 uF, L5 22 mH, C6 347 uF, D, B·C1 67.27 uF, B·L2 1.443 mH, B·L5 1.109 mH, B·C6 21.36 uF, B·L7 0.5728 mH, B·C8 5.739 uF, B·R9 3.739 Ω, B·D, C·C1 3.286 uF, C·L4 1.488 mH, C·L7 1.526 mH, C·C8 22 uF, C·L10 1.367 mH, C·C11 0.5021 uF, C·R12 19.93 Ω, C·D
- A5e.3c · 377.8 · 1947.9: G 2.83 V, L1 2.901 mH, C2 123.9 uF, L3 2.294 mH, C4 34.4 uF, L5 22 mH, C6 360.3 uF, L8 14.35 mH, C9 9.849 uF, R10 0.8577 Ω, D, B·C1 68.14 uF, B·L2 1.767 mH, B·C3 61.88 uF, B·L4 15.29 mH, B·L5 1.345 mH, B·C6 18.32 uF, B·L7 0.4131 mH, B·C8 5.071 uF, B·R9 2.301 Ω, B·C10 7.64 uF, B·L11 15.1 mH, B·C12 13.72 uF, B·L14 17.75 mH, B·C15 9.585 uF, B·R16 1.858 Ω, B·D, C·C1 12.41 uF, C·L2 2.007 mH, C·C3 18.74 uF, C·L4 0.9984 mH, C·R5 17.24 Ω, C·R6 47.64 Ω, C·L7 0.165 mH, C·C8 39.17 uF, C·R9 13.36 Ω, C·L10 0.4676 mH, C·C11 68 uF, C·R12 0.9358 Ω, C·D, C10 7.798 uF
- A5e.3c · 377.8 · 2304: G 2.83 V, L1 2.901 mH, C2 127.7 uF, L3 2.007 mH, C4 22.22 uF, L5 22 mH, C6 359.2 uF, L8 15.11 mH, C9 9.181 uF, R10 0.9871 Ω, D, B·C1 73.95 uF, B·L2 1.666 mH, B·C3 60.74 uF, B·L4 14.52 mH, B·L5 1.004 mH, B·C6 14.86 uF, B·L7 0.327 mH, B·C8 4.495 uF, B·R9 2.036 Ω, B·C10 22.77 uF, B·L11 7.148 mH, B·C12 26.7 uF, B·R13 0.8819 Ω, B·L14 2.938 mH, B·C15 0.9549 uF, B·R16 5.751 Ω, B·D, C·C1 6.858 uF, C·L2 0.3142 mH, C·C3 31.3 uF, C·L4 3.996 mH, C·R5 0.7236 Ω, C·R6 11.25 Ω, C·R7 3.213 Ω, C·C8 24.03 uF, C·L9 0.152 mH, C·C10 100.9 uF, C·R11 6.523 Ω, C·L12 2.961 mH, C·C13 5.6 uF, C·D, C10 68.08 uF
- A5e.3c · 549.7 · 1646.9: G 2.83 V, L1 2.717 mH, C2 109.2 uF, L3 1.128 mH, C4 23.19 uF, L5 22 mH, C6 355.1 uF, R7 0.284 Ω, D, B·C1 52.34 uF, B·L2 1.156 mH, B·C3 43.34 uF, B·L4 11 mH, B·L5 1.661 mH, B·C6 11.43 uF, B·L7 0.5899 mH, B·C8 3.276 uF, B·R9 6.228 Ω, B·L11 15.13 mH, B·D, C·C1 9.074 uF, C·L2 2.232 mH, C·C3 18.38 uF, C·L4 0.5782 mH, C·R5 0.9489 Ω, C·R6 47.01 Ω, C·R7 32.63 Ω, C·C8 17.44 uF, C·L9 0.06594 mH, C·C10 47.16 uF, C·R11 8.075 Ω, C·L12 0.864 mH, C·C13 33 uF, C·R14 0.9988 Ω, C·D, C10 93.04 uF
- A5e.3c (geleverd, niet bevroren) · 549.7 · 2304: G 2.83 V, L1 2.715 mH, C2 108.1 uF, L3 1.279 mH, C4 30.72 uF, L5 22 mH, C6 384.7 uF, D, B·C1 52.23 uF, B·L2 1.127 mH, B·C3 56.53 uF, B·L5 1.155 mH, B·C6 14.32 uF, B·L7 0.355 mH, B·C8 4.806 uF, B·R9 2.723 Ω, B·D, C·C1 3.341 uF, C·L4 0.6871 mH, C·L7 0.502 mH, C·L10 0.6115 mH, C·C11 1 uF, C·R12 17.1 Ω, C·D
- A5e.3b-arm · A5e.3b-arm bouwbare val (354,9 · 1994,6, L ≤ 22 mH): G 2.83 V, L1 2.514 mH, C2 160.9 uF, L3 1.659 mH, L5 22 mH, C6 377.1 uF, D, B·C1 53.53 uF, B·L2 1.512 mH, B·C3 74.59 uF, B·L5 1.105 mH, B·C6 13.38 uF, B·L7 0.2546 mH, B·R9 1.754 Ω, B·D, C·C1 5.6 uF, C·L2 0.2468 mH, C·C3 21.43 uF, C·L7 0.7033 mH, C·L10 0.0667 mH, C·R12 5.397 Ω, C·D

**WAT DE TABEL ZEGT.** *De vloer en de tweeter tegelijk, op elf van vierentwintig, en op beide conventies.* Elf netwerken halen 2,55–2,62 Ω (zes binnen de 2 %-tolerantie, 2,55–2,58) mét tweeter −20,1 tot −28,6 en mid −19,6 tot −49,7 op hun EIGEN kruispunten, zonder R of pad op de woofer — het koper op de wooferweg is gemiddeld 1,1 Ω serie (Y = het geleverde totaal, op elf van elf) en 2,6–4,6 Ω in totaal mét de val. *De val zit op de cap:* op acht van de elf staat de valspoel EXACT op 22,00 mH (342–385 µF, f₀ 55–58 Hz) — de zoektocht wil méér inductie in de val dan het grootste enkele catalogusonderdeel, en de cap bindt; op de drie andere 7,8 / 13,6 / 17,6 mH (f₀ 44–57 Hz). Wat dat kost is gemeten in de ablatie van A5e.3b en hier opnieuw: de arm (bouwbare val, 22,00 mH) leest RMS 0,86 / 0,73 waar A5e.3-veld's vrije val (27,85 mH) 1,03 / 0,85 las, en het veld op 377,8 · 1948 met 22,00 mH 0,85 / 0,78 — de cap kost op de RMS niets. *Twee RMS-kolommen:* volle band 0,85–1,20 (A5e.3-veld 0,85–1,20, V51b 1,81–2,15, HUIDIG 1,42), vanaf 397 Hz 0,70–0,91 (A5e.3-veld 0,66–1,18, HUIDIG 0,58). *Fase:* M-K M-T 3,1–4,5° op zeven van elf (A5e.3-veld 3,5–12,7; HUIDIG 7,0), 6,8 op 178,4 · 1647 en 14,1–18,1 op drie (313,2 · 1647, 377,8 · 1647, 549,7 · 2304); W-M 2,3–14,9° op negen, 36,8° op 259,6 · 1948, en op 178,4 · 1647 GEEN oordeel (M-K laat geen enkel W-M-punt toe: het geleverde kruispunt ligt op 254 Hz en de mid staat daar binnen het overlapvenster nergens boven de stille-geestvloer — een lezing die het rapport eerlijk als afwezig geeft, V44). *Dissipatie en de heetste weerstand:* 14–26 % (A5e.3-veld 9–24, HUIDIG 46), heetste R 0,62–1,80 W bij 10 W tegen 5,0 toegestaan (A5e.3-veld 0,31–1,93, HUIDIG 2,55), altijd het mid-pad B·R9 op één na (C·R5 op 259,6 · 1948). *Onderdelen en BOM:* 20–40 onderdelen (A5e.3-veld 18–35, HUIDIG 23, de arm 18), € 222–492 uit de catalogus (A5e.3-veld 267–460, de arm 212, HUIDIG 142) — over het corpus staan 14 vallen en 37 gedempte vallen (A5e.3-veld 4 en 17 over zeven netlists), en dát is het verschil met de arm: dezelfde RMS met de helft van de onderdelen, want de arm is een WAARDEN-hertune op de topologie van A5e.3-veld's 354,9 · 1995 (18 onderdelen) en het veld koopt zijn correctiegroepen opnieuw. Twee BOM-realisaties zijn ONDERGRENZEN (een sub-ohm-weerstand of een grote elco zonder onderdeel binnen ±5 %), en de spoelen van 12,9–18,8 mH zijn stapels van twee: de 1,4 mm-familie dekt dat bereik niet met één onderdeel binnen ±5 % — een catalogusfeit, geen zoekregel (`catalogSnap: false` op de v2-route). *Lobing-synthese* −1,6 tot −2,9 dB (A5e.3-veld −1,6 tot −3,2, HUIDIG −2,9). *De kruispunten drijven:* de tuner legt de W-M-overname 0–130 Hz BOVEN de gestelde positie (178 → 254, 215 → 281, 260 → 295–341, 313 → 341, 378 → 432–457, 550 → 535–553) en de M-T-overname 0–230 Hz ERONDER (1647 → 1637–1772, 1948 → 1717–2043, 2304 → 2049–2243); wat geleverd is kruist op 254–553 · 1637–2243, en de gestelde positie is de start en niet het antwoord. *Gepaard op het dichtstbijzijnde A5e.3-veld-kruispunt* (afstand 0,07–0,43 octaaf): min |Z| −0,27..+0,00 (de 2,85 van 229,1 · 1727 is weg — dat was de L+R-wees), RMS volle band −0,28..+0,30, tweeter M-C beide kanten op (−8,25 op 377,8 · 1647 tegen 354,9 · 1495; +1,18 op 259,6 · 2304), dissipatie −6..+13 punten, onderdelen −8..+21 — een anekdote per rij; het corpusgemiddelde (W-M-fase 8,6 → 9,2°, dissipatie 16,2 → 20,5 %) is compositie en geen delta (de V47-nazorg-leesregel, hier zonder één paar). *HUIDIG, met pad, op zijn eigen 360 Hz:* 3,46 Ω, 0,58 vanaf 397 (nog steeds het vlakste van allemaal) maar 1,42 op de volle band, 21° W-M-fase, 46 % dissipatie, 2,55 W in R8.

**DE TWEE VRAGEN.**

*1. Bestaan er lage W-M-kruisingen (148–260 Hz) met bouwbare spoelen?* **Ja — vastgelegd: lage W-M-kruisingen met bouwbare spoelen bestaan (178–378 Hz geleverd, en 550; L5 ≤ 22 mH op élk), alleen 147,9 — de positie óp de aandrijfvloer van de mid — valt, en wel op de versterkervloer met Y onoplosbaar; geen W-M-ondergrens uit bouwbaarheid nodig. De bouwbaarheid is niet wat de lage kruisingen begrenst.** Op 178,4 Hz levert 1647 (val 7,77 mH / 1531 µF, 37 onderdelen, RMS 1,20 / 0,77, tweeter −20,1 — de rand), op 215,2 Hz levert 2304 (17,57 mH / 743 µF, 40 onderdelen, RMS 1,07 / 0,70, M-K 2,3 / 3,1° — de laagste fase van het veld), en op 259,6 Hz leveren ALLE DRIE de M-T-posities (22,00 / 18,76 / 22,00 mH; RMS 0,97–1,11; 259,6 · 2304 met M-K 2,7 / 3,6° en 0,74 vanaf 397 Hz). Elke spoel op de laagste weg ligt binnen de spanwijdte — dat is wat de cap doet — en de vloer wordt op koper alleen gehaald (1,00–1,20 Ω). Wat de lage kruisingen WEL begrenst staat in de weigeringen, en het zijn drie andere dingen: (i) de VLOER op 147,9 Hz — alle drie op 2,23–2,52 Ω met Y ONOPLOSBAAR (20 Ω aan de kop van de woofer laat het minimum in de mid- of tweetertak staan; op 147,9 · 1647 staat de valspoel al op 22,00, op de twee andere op 7,6–9,0 mH), dus een LR4 op 148 Hz laat de mid tot ver onder haar doorlaatband meelopen en het minimum zit in háár ladder; (ii) het RIMPELDOEL onder `'none'` (178,4 · 1948 met 0,08 dB, 215,2 · 1647 met 1,25 dB: X = 0,78 dB niveauwerk dat de configuratie vraagt en niet krijgt); (iii) de TWEETER op de eigen kruispunten (215,2 · 1948, stap 2). Geen van de drie is bouwbaarheid. **Voorstel, niet gebouwd: GEEN W-M-ondergrens uit bouwbaarheid** — de gemeten ondergrens ligt op ~178 Hz en wordt door de vloer en de topologie-eis gezet, niet door de spoelen; een bouwbaarheidsgrens zou posities verbieden om een onderdeel dat elke positie evenveel nodig heeft (de A5e.3b (b)1-redenering, nu gemeten). Wat de cap wél zegt: de val wil op acht van elf netwerken groter dan 22 mH, en of dat een STAPEL mag worden is de gestelde uitzondering die niet gesteld is — de ablatie en dit veld zeggen allebei dat de cap op de RMS niets kost, dus er is geen meting die om die uitzondering vraagt.

*2. Hoeveel van het veld haalt alles, en welke drie eerst?* **Elf van vierentwintig halen alles** (vloer, M-C op zaad én eigen kruispunten per weg, niveauwerk `'none'`, M-A/part bij 10 W); tien zijn bevroren, één (549,7 · 2304) niet door de shortlist-grootte. Dertien geweigerd: vier op de vloer (147,9 ×3 en 178,4 · 2304 — allemaal Y onoplosbaar), vier op het rimpeldoel (X = 0,78), twee op de L+R-wees, drie op de tweeter op de eigen kruispunten. **De drie om als eerste te bekijken, op RMS, fase, lobing en onderdelen:** (1) **377,8 · 1948 (KAND-V2-1)** — de vlakste van het veld op de volle band (0,85 / 0,78), M-K 4,9 / 3,8°, lobing −1,9 dB, tweeter −23,7, min |Z| 2,55 (binnen de 2 %-tolerantie — en dat is openstaand punt 7: de barrière las die smalle dip op 416 Hz als 2,63 en mikte 0,08 Ω te hoog), 37 onderdelen, € 456: het referentie-ontwerp van dit veld. (2) **377,8 · 1647 (KAND-V2-3)** — 0,92 / 0,78 met TWINTIG onderdelen en € 237, de meest beschermde tweeter (−28,6), maar M-T-fase 16,1° (geleverd M-T-kruispunt 1699 Hz): de eenvoudige, goedkope variant — de arm van A5e.3b (18 onderdelen, 0,86 / 0,73, 3,9 / 4,7°, € 212) staat er direct naast als bewijs dat die eenvoud de fase niet hoeft te kosten. (3) **259,6 · 2304 (KAND-V2-6)** — de lage W-M-kruising die het best leest: 0,99 / 0,74 (het vlakste vanaf 397 Hz van het veld), M-K 2,7 / 3,6°, tweeter −23,4, opslingering −0,42 (de minste demping van de val: 22,00 mH / 561 µF), 38 onderdelen, € 492 (de duurste). Wie de lage overname om de directiviteit wil, kijkt hier; wie de rekening wil, bij (2); wie de vlakste som wil, bij (1). Ernaast, als vierde: 377,8 · 2304 (0,86 / 0,80, 3,2 / 4,5°, lobing −2,7 — het spiegelbeeld van (1) op de hoge M-T-positie).

**WAT ER GEBOUWD IS.** 1. **De weigering op de eigen kruispunten** (`worker.ts`: `by: 'derived-gate'`, `kinds: ['gate']`, `derivedGateViolation` als exporteerbare beslissing — één callback voor beide conventies, de bevroren eerst; een bevroren fout blijft de weigering van de tuner zelf). Byte-baselines (`f4cRegression`, `workerRouteRegression`) reproduceren: zonder gewapende poort vuurt zij nooit (P2).
2. **Het corpus:** `A5E3VELD_KAND_1..7` bevroren (`freeze-live-corpus.ts`, `a5e3veld_corpus`, `DATED_REASON.A5E3VELD`, corpus-id `a5e3veld` in `casus1Corpora.fixture.ts`); het levende corpus tien netlists (`KAND-V2-1..10`), geregenereerd op het A5e.3b-veld en met de drie geraakte kandidaten herhaald (`V2_ONLY` + `V2_MERGE=1`); de recorder over alles (tien levende blokken, zeven gedateerde, `v33_barriere_raster` op het verlengde raster, `v51_niveauwerk` met een LEGE wezenlijst op het levende corpus — zoals A5e.3b voorspelde).
3. **De generator** schrijft het twaalfde besluit (`spoel_spanwijdte_plafond_mH`, `spoel_spanwijdte_plafond_waarom`) en de veldnoot noemt de `drive-stated`-vloer.
4. **De scripts:** `measure-a5e3c-field.ts` (deze tabel: uitkomst met grond, kruispunt gesteld → geleverd, koper per weg, val, onderdelen, BOM uit de catalogus, de shards als derde toestand, de twee vragen als samenvatting; `casus1_a5e3c_veld_tabel.json`), `measure-a5e3-field.ts` herankerd op `a5e3veld`.
5. **De tests:** `derivedGateRefusal.test.ts` (nieuw, negen claims), `corpusPairing` (herankerd + de A5e.3c-claim), `frozenNetlistGates` (de twee A5e.3b-veldclaims herankerd op het levende veld en op het gedateerde blok), `casus1V2Candidates` en `casus1V2Refusal` (de A5e.3b-pin eruit: strikte vorm terug, en beide live ketenruns draaien weer — de byte-reproductie op KAND-V2-1 = 377,8 · 1948, de verwerping op 147,9 · 1647).

**TESTS.** Nieuw of veranderd: `derivedGateRefusal.test.ts` (nieuw, negen claims: de beslissing als functie; op de echte bestanden een referentie bevroren van HUIDIG tegen het gedateerde A5e.3-veld-corpus waar een grens tussen de twee lezingen op de ene conventie gehaald en op de andere gemist wordt, mét de tegenproef; de V32-vorm voor M-C op de corpora die de weigering beheerst; de zestien gedateerde afwijkingen als benoemde lijst; en de acceptatie dat élke levende netlist M-C op zijn eigen kruispunten haalt), `corpusPairing` (de A5e.3-veld-claim herankerd op `v51b → a5e3veld`; de A5e.3c-claim: nul paren op label, tien uit elf), `frozenNetlistGates` (de twee A5e.3b-veldclaims herankerd; de V33-guard boekt het resolutieverschil van KAND_V2_1 bij naam en eist gelijke oordelen; de V38-fix-rangclaim op een BENOEMDE verzameling — zij viel voor de tweede keer in de V47/V48-val toen `A5E3VELD_KAND_*` het complement binnenstapte, rang 75 van 150), `casus1V2Candidates` en `casus1V2Refusal` (de A5e.3b-pin eruit; de meetopstelling leest `'safety-extended'`; de `it.each` over het levende corpus loopt over TIEN netlists), `goldenClassification` (zeventien blokken erbij, automatisch), `casus1Field`, `ciLayer`, `choiceKeyGuard`, `p6Lint`, `noWeights`, `toggleRegression` ongewijzigd groen; de byte-baselines (`f4cRegression`, `workerRouteRegression`) reproduceren onder de gewijzigde worker (P2). `tsc -b` (ook `scripts/`) groen. **Snelle laag 06-09-2026: 386 s — 152 bestanden (149 groen, 2 rood, 1 overgeslagen), 1710 tests (1705 groen, 3 rood, 2 overgeslagen)**; de drie rode claims (meetopstelling, V33-resolutie, V38-fix-rang) zijn hierboven beschreven, gerepareerd en apart groen (frozenNetlistGates 358 s, derivedGateRefusal 105 s). **Volle run 06-09-2026, alleen gedraaid ná de recorder en de reparaties, niets ernaast: 152 bestanden, 1710 tests, 7194 s (1 u 59 min 54), niets overgeslagen, in één keer groen.** De wandkloktijd IS de byte-reproductie van KAND-V2-1 (377,8 · 1948: 7182 s live — dezelfde kandidaat kostte in de generator 8671 s onder belasting en 647 971 evaluaties), de verwerpingsrun van 147,9 · 1647 kostte 2136 s ernaast; de rest van de suite draait in de schaduw (`lowestWayLevelWork` 379 s, `frozenNetlistGates` 358 s, `threeWayChain` 317 s). Beide live ketenruns draaien dus weer, en de suite is navenant DUURDER dan de 359 s van A5e.3b — dat was de tijdelijke prijs van de parkeerpin, en dit is de prijs van het bewijs.

**WAT ER NIET VERANDERD IS.** Geen metriek, geen inversie, geen grens: vloer 2,6, opslingering 1,4, Q_es 2,4, M-C
−20 / afgeleid, 10 W, plateau 0, `'none'`, de spanwijdte-cap, de barrièrebron, level-work/1.2 — alles zoals A5e.3b
het achterliet. De ENE poortwijziging is een handhaving en geen herdefinitie: het derived-oordeel bestond al en
weigert nu (V31).
